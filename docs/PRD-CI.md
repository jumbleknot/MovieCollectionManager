...
| **TLS / DNS** | TLS at the Cloudflare edge **or** Caddy + Let's Encrypt DNS-01 (Cloudflare); `grumpyrobot.co` DNS on Cloudflare — no DDNS needed. |
...
- Prod APK baked to `https://app.grumpyrobot.co` (public host, HTTPS) — **not** an IP or `:8082`.
- Keycloak prod mode: `KC_HOSTNAME=auth.grumpyrobot.co`, proxy headers, real SMTP, brute-force on, admin console not public.
...
- BFF issuer/`ROOT_URL` → public `auth.` origin; session cookie `Secure`+`HttpOnly`, domain `app.grumpyrobot.co`; CORS limited to the app origin.
