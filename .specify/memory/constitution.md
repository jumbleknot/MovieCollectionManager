...
- v2.4.0: Test Type Integrity — sanctioned exception recorded for the agent golden tier (`golden`-marked model-decision tests MAY substitute the LLM dimension via the record/replay cassette seam). Scopes an existing prohibition; does not reverse or weaken it, and no other substitution is permitted anywhere. Raised by specs/048-test-harness-remediation analysis. (2026-08-07) [CURRENT]
-->
...
**Sanctioned exception — the agent golden tier (v2.4.0):** Tests carrying the `golden` pytest marker are **model-decision** tests, not integration tests, and MAY substitute the LLM dimension via the record/replay cassette seam (`agents/movie-assistant/src/eval/cassette.py`). They reside physically under `tests/integration/` only because the runner selects by marker (`pytest tests/integration -m golden`); the live-key integration run excludes them with `-m "not golden"`, so the two selectors are complementary and neither tier is diluted. This is the **only** sanctioned substitution anywhere in the repository, and it covers the **LLM dimension only** — every other collaborator (MongoDB, Keycloak, Redis, MCP servers, mc-service, TMDB) stays real in every tier. Cassettes are keyed on `sha256(model_id + normalized prompt)` so a prompt or model change produces a loud miss, never a stale pass; a cassette miss MUST fail the run and MUST NOT be converted to a skip. Extending this exception to any other dependency requires a further constitution amendment.
...
├── README.md
├── docs/
│   └── ...  # Human-readable documentation, such as user guides, tutorials, and general project information - HUMAN-OWNED: no generator may rewrite these
├── openwiki/
│   ├── INSTRUCTIONS.md  # Hand-authored generation brief - scope, exclusions, and redaction rules; never rewritten by the generator
│   └── ...  # GENERATED agent-facing knowledge wiki in the Open Knowledge Format (OKF): one markdown file per concept with queryable YAML front matter, an index.md per directory, and a resource link to the authoritative source in docs/ or specs/. A navigation and gotcha layer over the canonical documentation - it summarizes and links, never restates or replaces it. Approved deviation, 2026-07-26 (feature 043): kept separate from docs/ so the ownership rule stays enforceable - everything under openwiki/ may be regenerated by a tool, everything under docs/ is human-owned. Nesting it inside docs/ would put generator-rewritable files beside the precision operator runbooks and destroy that review rule.
├── specs/
