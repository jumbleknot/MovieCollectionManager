...
- **AI-assisted coding runs on the Claude MAX SUBSCRIPTION; the Anthropic API key is ONLY for the movie assistant and OpenWiki** — this holds identically on the Windows host, in the Docker Desktop dev container, and in the Docker Sandbox VM. The key is carried as **`MCM_ANTHROPIC_API_KEY`** and mapped to `ANTHROPIC_API_KEY` **only at the point of use** — the agent gateway (`agent-stack.mjs`), OpenWiki maintenance (`wiki-maintain.mjs`), and the containerized web/agent E2E recipe — each a separate process running no assistant. **Never set `ANTHROPIC_API_KEY` in any of the three environments**: Claude Code silently prefers it over an existing subscription login, with no warning and nothing in the UI showing which is in use, so a session bills pay-per-token while a valid subscription sits unused — **~$15 of unintended spend in one day, measured 2026-08-16**, on a workstation where `oauthAccount` was present throughout. CI is unaffected: it injects `ANTHROPIC_API_KEY` into jobs that run no interactive assistant, and every consumer honours that name first — [the mechanism, the consumer table, and the host migration command](docs/runbooks/devcontainer.md)
...
- **Before believing a test result, check the INSTRUMENT — most of this thread's wrong turns were instruments, not code** — a "deterministic reproduction" ran against a gateway that reported `running` at 100% CPU and answered nothing (#179, fixed); `--grep-invert` is accepted by Playwright 1.60 and silently does nothing here, so a CLI-based test split would have run everything and looked correct; the `git credential fill` token returns an **empty package list** rather than a 403, which reads as "no bundle was published"; bundle names come from `GITHUB_RUN_ID`, not `run_number`, so a constructed name 404s for a bundle that exists; `awk 'length>100'` counts BYTES, so every em-dash over-reports by two; and **`node --test <file> --test-name-pattern "x"` silently runs EVERYTHING**, because everything after the script path becomes the script's own `argv` — the natural-looking form, which will quietly turn a Verify RED into "all green" while filtering nothing (put node's flags BEFORE the path, or use `--test` with a glob). Each produced a confident wrong answer in minutes — [the instrument checks, and what a green tick does and does not prove](docs/runbooks/e2e-testing.md)
- **The agent E2E suite is SPLIT: model-decision assertions do not block a merge** — a required gate that fails on identical code roughly half the time is not gating. Measured: two runs of the same sha gave `failed=0` then `failed=1 flaky=7`, with contention, shared identity, the collapse and the gateway livelock each excluded by a counter. Agent tests carry `@gate` or `@model-decision`; the selection is `E2E_TIER` in `playwright.config.ts` (**not** `--grep-invert`), and an unclassified test FAILS rather than defaulting into a tier. The model tier still runs on every `main` push and dispatch, non-blocking, publishing its own counts — nothing leaves the gate without a tier that runs it — [the rule, the classification, and what the gate stops proving](openwiki/invariants/testing-tiers.md)
- **Run the test tiers your DIFF touches, not the ones you remember** — `affected` caught eight ruff findings in Python this session because `nx test movie-assistant` was run and `nx lint movie-assistant` never was. Derive the checks from what changed; a tier you did not think of is a tier that did not run — [the tiers and what gates a merge](openwiki/invariants/testing-tiers.md)
- **A guard that fails because you changed what it protects gets UPDATED at the cause, never deleted** — raising the worker ceiling broke `workers <= cap - 3`, which existed only because workers shared one user; it now asserts the new premise (that setup mints an identity per worker) so it still fails if that regresses. A guard removed for being inconvenient is the regression it was written to catch — [the invariants and where each is enforced](openwiki/invariants/testing-tiers.md)
- **Renovate's silence is not idleness — a channel with a missing toolchain dies quietly, and the nightly cron is NOT the Friday window** — routine branch work returns `not-scheduled` outside `* 2-4 * * 5`, so "it will sort itself out tonight" is false for everything except a security PR or a dashboard tick; and a channel whose lockfile binary is absent has its `execa` rejection SUPPRESSED to one dashboard line while producing nothing, for ever — the `pep621` channel had never once run (item #218). A forced dispatch defaults to `dryRun: true` and its mode is unreadable on this forge, so confirm live-vs-dry from branch heads moving and ticks being consumed, never from a green run — [the schedule, the dashboard checkbox table, forcing a run, and the silent failure modes](docs/runbooks/renovate.md)
- **When the whole CI board goes red, suspect the INSTALL before the code** — six required contexts died together on a one-line dependency bump because a supply-chain policy rejected a transitive published 1.7 hours earlier; `okf` and `sast` failing beside `affected` is the tell, and Renovate's 3-day cooldown covers what it PROPOSES, never what a lockfile regeneration drags in. Same shape as the superseded-run trap, different cause, and reading all six digests is wasted effort — [the two causes of a whole-board red, and the diff question that tells a superseded branch from a live one](docs/runbooks/ci-diagnostics.md)
...
- **Nx as the universal task runner** **[canonical]** → [openwiki/invariants/nx-task-runner.md](openwiki/invariants/nx-task-runner.md) — Why nearly every build/test/lint/deploy command across the polyglot monorepo goes through Nx rather than the underlying tool directly, the executors that bridge Nx…
...
- **The agent-driven backlog (Forgejo Issues)** → [openwiki/runbooks/backlog.md](openwiki/runbooks/backlog.md) — How the MCM backlog lives in the repository's own Forgejo tracker and is worked by the coding assistant through scripts/backlog.mjs — the credential whose reach is bounded by a client-side write guard rather than by its…
...
- **Containerized dev environment (devcontainer)** → [openwiki/runbooks/devcontainer.md](openwiki/runbooks/devcontainer.md) — The disposable Linux dev container the AI coding assistant runs inside. **As of feature 060 it is hosted on a Docker Sandbox microVM, not Docker-in-Docker** — the operating manual is [docs/runbooks/devcontainer-sandbox.md](docs/runbooks/devcontainer-sandbox.md) (lifecycle, host-policy-first egress triage, the socat seam that returns exit 0 with empty output, disk limits). The Docker Desktop/DinD path is **retained solely for the Android emulator**, which needs `/dev/kvm` the microVM cannot provide…
...
`specs/063-infra-image-version-pins/plan.md`
<!-- SPECKIT END -->
