...
- **"It can't run here" is a claim to verify, not one to inherit** — the dev container's egress allowlist blocks particular **tools**, not whole capabilities, and the difference is where confidence goes to die. `pnpm nx e2e mcm-app` genuinely cannot run (chromium is uninstallable), but web and agent E2E run fine in the official Playwright image with `--network host` — the recipe is committed, the image is pre-pulled, and the credentials are in `.env.e2e.local`. Before recording anything as unverifiable — in a handoff, a task note, a commit, or a message — run the runbook's recipe first. 047 PR B carried "Playwright cannot run in this container" through several commits and a handoff before anyone checked; the same shape as the two gates above, where a tool answering "no" for an environmental reason was read as the answer to a different question — [the recipe, and the `--user` flag that is not optional](docs/runbooks/devcontainer.md)
- **A skipped test reads as a pass — never quote a green run as verification without the SKIP COUNT** — every tier here skips silently when its dependency is down: the Python integration tiers skip the unreachable MCP server, and all ten `agent-*.spec.ts` skip without the containerized stack. `89 passed / 17 skipped` was reported as verified when the full stack gives `95 passed / 11 skipped`, and one of those six had genuinely broken and reached CI. Run with **`MCM_REQUIRE_LIVE_STACK=1`** (Python) and **`E2E_REQUIRE_AGENT_STACK=1`** (Playwright), which turn a non-allowlisted skip into a failure naming what is down — [the no-false-green rule and the per-tier gates](openwiki/invariants/feature-validation-checklist.md)
