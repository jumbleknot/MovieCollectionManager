...
- **Debugging an `app-ci / app-e2e` agent-flow failure**: download the **`agent-e2e-container-logs`** artifact (full timestamped gateway/MCP/BFF/mc-service/keycloak logs, uploaded on failure before teardown) — it is the authoritative server-side evidence; the API exposes no step logs and the container is gone after the `always()` teardown. An empty dock panel at failure is the dropped-`/run`-response signature, not model slowness. Recent app-e2e is genuinely green, so a lone agent-flow failure is a **real anomaly to diagnose from the artifact**, not noise to re-run away. Procedure: [e2e-testing.md](docs/runbooks/e2e-testing.md) (diagnosis step 6).
...
`specs/035-infra-image-cve-scan/plan.md`
<!-- SPECKIT END -->
