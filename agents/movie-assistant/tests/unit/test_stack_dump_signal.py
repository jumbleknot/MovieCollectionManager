...
These tests pin the mechanism itself, not the defect. A diagnostic first exercised during an
incident is a diagnostic that fails during an incident.
"""
...
def test_dump_carries_frames_but_no_environment() -> None:
    ...
    `faulthandler` prints frames only — no locals and no environment — which is what makes it
    safe to leave armed permanently. Asserted rather than assumed, because "it does not log
    secrets" is exactly the claim that needs a test rather than a comment.
    """
    ...
    # `io.UnsupportedOperation: fileno`. The constraint that makes the mechanism suitable is
    # the same one that shapes its test.
    ...
    # A dump that included locals or the environment would leak whatever the process holds.
    # Probe for the shapes that would actually appear if it did.
