...
- [X] T032 [P] Implement `backend/mc-service/src/application/commands/set_default_collection.rs`: `SetDefaultCollectionCommand` + Handler; clears previous default then sets new one via two sequential repository calls (`clear_default_for_owner` + `set_as_default`) — no transaction used; a crash between the two calls leaves no default set, which is a safe and retryable state; pass T031 (GREEN)
...
- [X] T164 [P] Write load test in `frontend/mcm-app/tests/load/` that seeds a 10,000-movie collection and asserts: initial list load time <3s (SC-006), search response time <3s (SC-006), home screen collection list load time <3s (SC-004); add `test:load` Nx target to `frontend/mcm-app/project.json` (executor: `nx:run-commands`, command: `node tests/load/run.js` or equivalent k6/artillery invocation); run via `pnpm nx test:load mcm-app`. Note: SC-005 ("create collection + first movie in under 3 minutes") is a user-workflow time budget that cannot be validated with a server-side load test — it is validated via manual exploratory testing of the create-collection and add-movie flows.
...
- [X] T166 Fix non-atomic cascade delete to prevent orphaned movie records on partial failure:
  1. **Fix incorrect comment** in `backend/mc-service/src/application/commands/set_default_collection.rs` line 26 — remove the false claim "MongoDB adapter uses a session transaction"; neither `set_default` nor `delete` uses a real transaction
  2. **Enable single-member replica set** in `infrastructure-as-code/docker/mc-service/compose.yaml`: add `command: ["--replSet", "rs0"]` to mc-db; add a one-shot `rs-init` service that runs `mongosh --eval "rs.initiate()"` with a `depends_on: mc-db: condition: service_healthy` and `restart: on-failure`; add `--replSet rs0` URL param to mc-db `MC_DB_URL` in mc-service env; update integration test compose if separate
  3. **Wrap delete in a transaction** in `backend/mc-service/src/adapters/mongodb/collection_repository.rs`: add a `client: mongodb::Client` field to `MongoCollectionRepository`; in `delete`, start a `ClientSession` with `client.start_session()`; run `delete_one` + `delete_many` inside `session.with_transaction(...)` so both writes are atomic; abort on any error; update `delete_cascades_to_movies` in `backend/mc-service/tests/integration/collections/delete_test.rs` to seed 3 movies and assert `movies.count_documents({ collectionId })` returns 0 after delete (stronger than asserting a single movie's absence); add a second integration test `delete_is_atomic_on_cascade_failure` that verifies aborting the transaction mid-way leaves both the collection document and all movies intact (use a separate non-transactional session to verify counts)
  4. **Fallback orphan log on startup** (defence-in-depth regardless of transaction support): in `src/main.rs` after MongoDB connect, run the orphan-detection aggregate (`movies` where `$lookup` from `movie_collections` returns empty array) and emit `tracing::warn!` with the count if > 0
- [X] T167 [P] Write unit tests (RED) for `frontend/mcm-app/src/components/unit-tests/no-autofill-input.test.tsx`: (1) renders without errors, (2) forwards testID prop, (3) forwards value and onChangeText props, (4) forwards placeholder prop, (5) injects `autoComplete="off"` when `Platform.OS === 'web'`, (6) does NOT inject `autoComplete` override when `Platform.OS === 'android'`, (7) does NOT inject when `Platform.OS === 'ios'`
- [X] T168 [P] Create `frontend/mcm-app/src/components/no-autofill-input.tsx`: thin wrapper around `TextInput` that injects `autoComplete="off"`, `data-form-type="other"` (Dashlane), `data-lpignore="true"` (LastPass), `data-1p-ignore=""` (1Password), `data-bwignore="true"` (Bitwarden) only when `Platform.OS === 'web'`; transparent pass-through on native (no extra props); update `frontend/mcm-app/src/components/collection-form.tsx`, `frontend/mcm-app/src/components/movie-form.tsx`, and `frontend/mcm-app/src/components/movie-search-bar.tsx` to replace all `<TextInput>` usages with `<NoAutoFillInput>`; pass T167 (GREEN) [FR-026]
- [X] T169 [P] Extend unit tests in `frontend/mcm-app/src/components/unit-tests/movie-detail.test.tsx` (RED): "renders a tappable URL link for external IDs that have a url" (expects testID `movie-detail-ext-id-url-0` present), "does not render a link element when external ID has no url" (expects null); implement in `frontend/mcm-app/src/components/movie-detail.tsx`: for each `externalId` entry, render the system+uniqueId text plus a `<TouchableOpacity accessibilityRole="link">` wrapping the URL text when `eid.url` is set; URL handler uses `window.open(url, '_blank', 'noopener,noreferrer')` on web and `Linking.openURL(url)` on native; pass updated tests (GREEN) [FR-014a]
- [X] T170 Write integration test for large movie collection in `backend/mc-service/tests/integration/movies/large_collection_test.rs`: seed 100 movies across 10 distinct attribute groups (each group of 10 with unique: contentType, language, genre, decade, owned, ripped, childrens, rated, ownedMedia, ripQuality, title/plot/director/actor/tag values); assert pagination returns 50 movies in page 1 + 50 in page 2 with null final cursor; assert each individual attribute filter returns exactly the matching 10-movie subset; assert text search across title/originalTitle/director/actor/tag/outline/plot/movieSet returns correct subset; assert combined search+filter intersection; assert `filter_options` endpoint returns exactly the seeded distinct values; fix MongoDB text index to add `default_language("none")` + `language_override("textSearchLang")` to prevent WriteError 17262 on non-European language values (e.g., "Japanese", "Korean") in `backend/mc-service/src/adapters/mongodb/indexes.rs`
