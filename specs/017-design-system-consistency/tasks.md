...
- [x] T001 Confirm branch `017-design-system-consistency` is checked out and the dev-container backend stack is reachable (`docker compose --profile bff-dev up -d`); confirm the merged audit specs exist (`frontend/mcm-app/tests/e2e/web/{a11y,responsive,font-fallback,perf}.spec.ts`) and the DS unit suite runs (`pnpm nx test design-system`).
- [x] T002 Re-baseline the stable-selector snapshot: regenerate the testID inventory from current `frontend/mcm-app/src` and confirm it matches the committed selector baseline — recorded as `specs/017-design-system-consistency/selector-baseline.txt` (234 static literal testID call-sites across 39 non-test files + 4 props-forwarded dynamic sites) to diff against at completion (FR-013/SC-006). (No prior committed baseline existed; this establishes it.)
...
- [x] T004 [P] Write the DS success-token contrast test in `packages/design-system/components/__tests__/success-token.test.tsx` (or colocated `theme.test.tsx`): assert `success`/`onSuccess`/`successContainer`/`onSuccessContainer` exist in `lightColors` + `darkColors`, resolve via `TamaguiProvider` under both themes, and meet AA (`success`-on-surface, `onSuccess`-on-success, `onSuccessContainer`-on-successContainer ≥ 4.5:1) in both themes — per [contracts/success-token.md](contracts/success-token.md). **Verify RED**: `pnpm nx test design-system --skip-nx-cache` → fails (roles absent).
- [x] T005 Add the `success` green tonal ramp to `packages/design-system/tokens/palette.ts` and the four success roles to `lightColors`/`darkColors` in `packages/design-system/tokens/colors.ts` (light 40/100/90/10, dark 80/20/30/90), wire them into `lightTheme`/`darkTheme` in `packages/design-system/theme.ts` and the `colorTokens` map in `packages/design-system/tamagui.config.ts` (so `$success` resolves). **Verify GREEN**: T004 passes (tune tones if a contrast assertion fails).
- [x] T006 [P] Add Inter SemiBold (600) + Inter Bold (700) faces to `packages/design-system/fonts/index.ts` (`interFont` `face` map) and to the app `useFonts` map in `frontend/mcm-app/src/app/_layout.tsx`, so declared Inter 600/700 weights render real faces (research D2). **Done when**: fonts load without warning and DS unit + app unit stay green.
- [x] T007 [P] Export the success roles' types/usage note from the DS barrel if needed and update `packages/design-system` README token list to include `success`. Roles flow through the existing `lightColors`/`darkColors` barrel exports (so `LightColors`/`ColorToken` types include them); created `packages/design-system/README.md` documenting the colour roles incl. `success` + usage. **Done**: DS tsc clean; DS unit 74/74 green.

### T003 — Static DS-compliance scan (RED driver) — DONE

- [x] T003 Authored `frontend/mcm-app/tests/unit/design-system-compliance.test.ts` (R1–R5). Verified RED: **R1=35, R2=12, R3=19, R4=23, R5=2** with per-violation `file:line — rule — snippet` lists. Comment-aware `neutralize()` keeps string contents (colour literals + `fontFamily` values stay scannable) while stripping comments; `ds-exempt(R<n>)` site annotations drive the sanctioned allowlist. Scan surfaced violations beyond the enumerated tasks — scrim-overlay `rgba` fallbacks (delete/logout dialogs), password-strength colours, the email-verification `64` icon + nav wordmark `20`, and R3 is 19 react-native `<Text>` styles (not 3) — all handled in-phase + annotated; tasks updated to stay SDD-aligned.
