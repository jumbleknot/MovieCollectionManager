...
- [ ] **T001** [US1] Add a `healthcheck` to the `movie-assistant-gateway` service in
...
- [ ] **T002** [US1] **Prove the probe works without waiting for a real wedge**: `docker pause` the
...
- [ ] **T004** [US1] Confirm the healthcheck does **not** silently restart the container: a wedged
...
- [ ] **T005** [US2] Install a `faulthandler` signal handler in `create_app()`
...
- [ ] **T006** [US2] **Prove the dump mechanism against a HEALTHY gateway** — `docker kill -s USR1`,
...
- [ ] **T007** [US2] Add a unit test pinning that the handler is registered and that the dump carries
      no environment or credential material. (FR-008)
- [ ] **T008** [US2] Write `scripts/capture-gateway-wedge.sh`: poll `/health`, and on the FIRST failure
