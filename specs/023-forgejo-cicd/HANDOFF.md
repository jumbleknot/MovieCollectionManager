...
**🏷️ Stack renames (user, 2026-06-29b)** — `prod-app → prod-mcm-bff`, `prod-data → prod-mc-service`, `prod-agents → prod-movie-assistant` (`prod-auth` unchanged). The plan/tasks now use the NEW names. **The live Komodo stack is still named `prod-app`** until the cutover. ⚠️ **Komodo overrides the compose `name:` with the Komodo Stack name** (confirmed live 2026-06-30 — deploying the `name: prod-mcm-bff` compose under the `prod-app` Stack redeployed in place as project `prod-app`). So the rename is driven by renaming the Komodo Stack / the T036 ResourceSync creating `prod-mcm-bff`, via a down-old/up-new cutover (`docker rm -f mcm-bff-service mcm-bff-cache-redis mcm-bff-store-mongo`; external vols/nets survive) — recommended to defer to T036 (one cutover, not two). Also confirmed: **REGISTRY_HOST must be in the Stack Environment field** (Komodo writes it to .env.prod); a bare global Komodo Variable does nothing unless referenced as `REGISTRY_HOST=[[REGISTRY_HOST]]`. Historical entries below say `prod-app` = today's live `prod-mcm-bff`.
...
**REPO-SIDE DONE (2026-06-30):** Milestone A (T029–T032, commit `f193c7f`) + Milestone B (T033/T034/T035) — both prod composes authored, gates green, `compose config` renders both stacks, web-api-mcp isolation verified, 4-stack TOML `after`-chain parses. **prod-mcm-bff is LIVE on the host-free digest** (operator wired REGISTRY_HOST + `.env.deploy`). cd-deploy writes the digest block to all 3 stack dirs. prod-realm.json already carries the token-exchange clients (§8 resolved).

**▶ NEXT — operator/runtime only:**
- **T036** ResourceSync bootstrap: register `mcm-repo` Repo + seed masked Komodo Variables (`BASE_DOMAIN`,`TAILNET_HOST`,`TS_ADMIN_IP`,`REGISTRY_HOST`,`agent_db_password`,`agent_gateway_client_secret`,`anthropic_api_key`(empty ok),`mcm_*` secrets — **NOT `mc_db_password`**); create the ResourceSync; rename live `prod-app`→`prod-mcm-bff` via down-old/up-new (Komodo Stack name, not compose `name:`).
- **T038** prod daemon: create `movie-assistant-mcp-network`; confirm vols `mc-service-store-mongo-data` + `movie-assistant-store-postgres-data`; set `AGENT_SUBJECT_TOKEN_CLIENT_SECRET` in the prod-mcm-bff Stack env (was empty).
- **T018/T019** validate `deploy=true` (signed webhook→probe→rollback — only unexercised CD leg) → **T021** branch-protection → **T037** merge 022→main → **T039** full-app verify (create-a-movie off-network + one agent flow + rollback drill).
- Drive `cd-deploy` via API dispatch (`write:repository` token; 🚨 revoke after) — UI won't list it until `.forgejo/workflows/*` is on `main`.
...
- **022 prod is LIVE**: `prod-auth` + **`prod-app` deployed via Komodo**; `https://auth.${BASE_DOMAIN}` + `https://mcm.${BASE_DOMAIN}` reachable through Cloudflare tunnel; BFF healthy + wired to Keycloak. **HEADLINE ACCEPTANCE MET (T028/Part D): off-network cellular login PROVEN on the prod APK** (and web). Komodo Stacks point at branch `022-prod-public-hostname-auth`.
- **Registry has all 6 images** (`<tailnet-host>:3000/jumbleknot/<img>@sha256`); `.env.deploy` committed with the digests. prod-app currently **hand-sets `MCM_BFF_IMAGE`** in the Komodo Stack Environment (works) — see digest-by-git wiring below for the steady state.
...
1. Operator sets Forgejo **`KOMODO_WEBHOOK_URL`** (var) + **`KOMODO_WEBHOOK_AUTH`** (secret, == Komodo global `KOMODO_WEBHOOK_SECRET`) for the prod-app Stack. (`BASE_DOMAIN` var already set = `${BASE_DOMAIN}`.)
