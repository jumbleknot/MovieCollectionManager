...
- [ ] T038 (operator) **M1 — pre-create external nets/vols on the prod daemon** for the new stacks. **Networks**: create `movie-assistant-mcp-network` + `mc-service-network` (the latter isolates mc-service's Mongo); confirm `backend-network`, `mcm-bff-network`, `edge-network`, `keycloak-network` already exist. **Volumes**: confirm `mc-service-store-mongo-data` + `movie-assistant-store-postgres-data` exist (runbook Phase 5 pre-provisioned them). `agent_db_password` must match what `movie-assistant-store-postgres-data` was first initialised with. **Also**: set `AGENT_SUBJECT_TOKEN_CLIENT_SECRET` in the prod-mcm-bff Stack Environment (was empty until agents deploy — the BFF mints the subject token) and `agent_gateway_client_secret` for prod-movie-assistant (from Keycloak admin → agent-gateway client Credentials). (work order §6.5)
