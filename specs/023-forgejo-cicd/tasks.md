...
- [ ] T036 [US3] (operator) **B1/B2/M3 — bootstrap the ResourceSync (replaces the per-Stack UI build of T013).** (1) Register the Forgejo git provider + create the `mcm-repo` **Repo** resource (holds the tailnet host). (2) Seed masked Komodo **Variables**: `BASE_DOMAIN`, `TAILNET_HOST`, `TS_ADMIN_IP`, `REGISTRY_HOST` (= `[[TAILNET_HOST]]:3000`), and each per-stack secret (`KC_DB_PASSWORD`, `KC_BOOTSTRAP_ADMIN_PASSWORD`, `mcm_keycloak_client_secret`, `mcm_keycloak_service_client_secret`, `mcm_cookie_secret`, `mcm_agent_config_enc_key`, `mcm_agent_subject_token_client_secret`, `mc_db_password`, `agent_db_password`, `anthropic_api_key`). (3) Create the **`ResourceSync`** → `resource_path = infrastructure-as-code/komodo`, enable its git webhook. **`prod-auth` adopts in place** (same name). **`prod-app` is RENAMED to `prod-mcm-bff`** — the sync will NOT find an existing `prod-mcm-bff` to adopt, so do a clean cutover: `docker compose -p prod-app down` (or delete the old Komodo Stack) **then** let the sync bring up `prod-mcm-bff`, else the new project collides with the old project's `container_name`-pinned containers (`mcm-bff-service`, …). Brief BFF downtime is expected during the flip. (4) Record the ResourceSync webhook URL/auth into Forgejo `KOMODO_WEBHOOK_URL` var + `KOMODO_WEBHOOK_AUTH` secret. **Verify** `[[VARIABLE]]` interpolation resolves in the field you use it (certain for stack `environment`; the `linked_repo` route avoids needing it in `git_provider`). (work order §6.4; supersedes T013)
...
- [ ] T033 [US3] **R1 — `mc-service/compose.prod.yaml` (`prod-mc-service`).** `name: prod-mc-service`. Services: `mc-service-store-mongo` (replica-set `rs0`, **no published port** in prod), `mc-service-store-mongo-rs-init` (one-shot), `mc-service` (image `"${REGISTRY_HOST}/jumbleknot/mc-service@${MC_SERVICE_DIGEST}"`, `MC_DB_URL` directConnection, `KEYCLOAK_URL=http://keycloak-service:8080`, no host port). External `backend-network` + `mc-service-store-mongo-data` volume (pre-created). `depends_on` rs-init `service_completed_successfully`. **RED→GREEN**: `compose config` resolves with the digest+REGISTRY_HOST set; aborts naming a missing `${VAR:?}`. (FR-015, FR-017; plan Addendum R1)
- [ ] T034 [US3] **R2 — `agents/compose.prod.yaml` (`prod-movie-assistant`).** `name: prod-movie-assistant`. Services: `movie-assistant-gateway`, `movie-assistant-store-postgres` (agent-db), `movie-assistant-mcp-movie`, `movie-assistant-mcp-webapi`, `movie-assistant-mcp-spreadsheet` — each `"${REGISTRY_HOST}/jumbleknot/<repo>@${<REPO>_DIGEST}"`. `MODEL_PROVIDER` + `ANTHROPIC_*` (prod = Claude, not Ollama), RFC 8693 token-exchange env (`AGENT_GATEWAY_CLIENT_ID/SECRET`). Nets: external `backend-network` + isolated `movie-assistant-mcp-network` (web-api-mcp egress-only, no backend). No published ports. **RED→GREEN**: `compose config` resolves; web-api-mcp is NOT on `backend-network`. (FR-015, FR-017; plan Addendum R2)
- [ ] T035 [US3] Extend `komodo/stacks.toml` with the `[[stack]]` blocks for **`prod-mc-service`** (`after = ["prod-auth"]`) and **`prod-movie-assistant`** (`after = ["prod-mc-service"]`), and set `prod-mcm-bff`'s `after = ["prod-mc-service"]` (T031 placeholder). Confirm prod-realm RFC 8693 token-exchange policy + OPA is wired for the **prod** clients (work order §8 open item). (work order §6.3, §2 ordering)
- [ ] T038 (operator) **M1 — pre-create external nets/vols on the prod daemon** for the new stacks (`movie-assistant-mcp-network`; confirm `backend-network`, `mc-service-store-mongo-data`, `movie-assistant-store-postgres-data` exist). DB passwords must match what the volumes were first initialised with. (work order §6.5)
