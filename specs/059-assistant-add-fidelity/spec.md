...
- **FR-021**: **Every** existing test that walks the add chain's fixed sequence of turns, at any tier, MUST be updated for the new question — not only the end-to-end coverage of the 047 ownership chain. An extra question ahead of the ownership question changes that sequence, so a test left unchanged is asserting a flow that no longer exists. A test MUST NOT be relaxed (for example, to accept "some stage") to make it pass: that removes the ordering guarantee the change is about.
  - *Widened 2026-08-14 after cross-artifact analysis.* The original wording named only the end-to-end tier, and the plan and task list followed it. Four tests in the agent's **integration** tier assert the chain's first pause directly and would have gone red mid-implementation with no task owning them.
