...
- [~] T005 Capture web E2E baseline + per-flow timings on SDK 55: `pnpm nx e2e mcm-app`. ⚠️ **NOT a clean baseline — re-capture required.** Verified completed run (92 tests, 4 workers, `--skip-nx-cache`): **87 passed / 1 failed / 4 flaky** in **13.0m** (normal ≈4.5m → Metro dev-server degradation, per project memory). The 1 hard failure (`movies.spec.ts:855` Autofill suppression) and all 4 flaky (`movies.spec.ts` filter/search exact-count) failed at `gotoHome` (line 79) — i.e. navigation/cold-compile **timeouts**, not assertion/functional failures. A trustworthy SC-006 web baseline needs a **fresh Metro restart + single-worker** run (documented ritual) before T026 can compare. CORRECTION: earlier edits/commits in this file cited "39 passed / 0 failed" and "44/29/19 timeouts" — BOTH were unverified and wrong; **87/1/4 @13m is the only result observed to completion.**
...
**Checkpoint**: SDK 55 baselines — unit/integration GREEN (mcm-app unit 804, BFF integration 45; mc-service 99 unit + 118 integration). Pre-existing flaky tampered-JWT test fixed (committed). ⚠️ Web E2E (T005) is **NOT clean** (verified 87 passed / 1 failed / 4 flaky @13m; failures are navigation timeouts from Metro degradation, not functional — needs fresh-Metro single-worker re-capture). expo-doctor (T007) baseline = 15/19 (4 known failures). Android (T006) DEFERRED (no emulator). **Web + Android baselines MUST be re-captured cleanly before the T026 perf gate.** Per the chosen scope ("Approve, commit, run baselines"), implementation STOPS here — before T013. The actual SDK 55→56 upgrade resumes via `/speckit-implement` on a fresh session/Metro.
