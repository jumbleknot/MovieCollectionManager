...
- [ ] T037 Measure the write path's added wall-clock across ~20 new `always()` steps, especially on the capacity-1 `kvm` runner. If not negligible, cap collection work. Record the measurement. ⛔ **BLOCKED on T027** — needs real runs to measure added wall-clock.
