...
- [ ] T003 [P] Capture offline API fixtures into `scripts/__tests__/fixtures/ci/` for deterministic tests: a `head_sha` runs listing, a commit-statuses payload with all-required-green, one with a skipped gated job, one with a **cancelled** run (all contexts reading `failure`), and one with a failing non-required context. Redact hosts in the fixtures themselves.
...
**Checkpoint**: `node --test scripts/__tests__/ci-digest-redact.test.mjs` green; `node scripts/ci-digest-redact.mjs --selftest` green; `node scripts/secret-scan.mjs` still exits 0 (refactor regression clean).
...
- [ ] T015 [US1] Add `--selftest` to `scripts/ci-status.mjs` asserting the same cases as T009/T011 plus host redaction and missing-scope messaging. **This is the CI-visible contract** — `node --test` never runs in CI.
...
- [ ] T022 [US2] Add `--selftest` to `scripts/ci-failure-digest.mjs` asserting the same cases as T017/T019.
