...
**Decision**: The committed `devcontainer.json` uses `build.dockerfile: Dockerfile` + `build.args: { "BASE_IMAGE": "${localEnv:MCM_DEVCONTAINER_IMAGE:mcm-devcontainer:local}" }`. The committed **thin** `Dockerfile` is `ARG BASE_IMAGE=mcm-devcontainer:local` / `FROM ${BASE_IMAGE}` (its sole job is to parametrize the base). The digest-pinned forge reference (`<forge-host>/<ns>/mcm-devcontainer@sha256:…`) is supplied at runtime via the `MCM_DEVCONTAINER_IMAGE` env var, sourced from a **gitignored** local env — the forge host literal never enters git (topology-scrub gate, same rule 037 uses for `FORGE_REGISTRY_HOST`).
...
| `mcm-rustup` | `/home/coder/.rustup` | `RUSTUP_HOME=/home/coder/.rustup` |
| `mcm-uv-cache` | `/home/coder/.cache/uv` | `UV_CACHE_DIR=/home/coder/.cache/uv` |
| `mcm-pnpm-store` | `/home/coder/.local/share/pnpm/store` | pnpm `store-dir` |
...
**Rationale**: Satisfies SC-005 (install from cache, 0 re-downloads) and FR-004. **Gotcha (verified)**: a named volume mounted over an image dir starts **empty and shadows** image contents — so these caches are **runtime-populated** (first `cargo build`/`uv sync`/`pnpm install` fills them; they then persist). Do NOT pre-warm a cache in the image under a mount point — it's hidden. **Ownership gotcha**: a fresh named volume is `root:root` and would deny `coder` first write; pre-`chown`ing the target in the image lets copy-up carry uid 1001 into the fresh volume. Belt-and-suspenders: a root `postCreateCommand`/`onCreateCommand` `chown -R coder:coder` fallback repairs a pre-existing root-owned volume.
