...
- [X] T020 [US2] Verify the fast path: set `MCM_DEVCONTAINER_IMAGE` to the pushed digest, recreate → run `verify-caches-persist.sh` **GREEN**; **time** warm recreate < 90 s with 0 toolchain re-compile (SC-003) and stop→start < 15 s (SC-004). Confirm the first-provision cost does not recur on subsequent opens (SC-011). **DONE 2026-07-14** (host-measured on the forge digest image): warm recreate ~17 s with **0 image-pull + 0 rustc/cargo compile** (docker create→start same second); stop→start **0.331 s**; `verify-caches-persist.sh` + `verify-toolchain-present.sh` PASS.
...
- [X] T025 [US3] With the dotfiles repo configured, recreate the container and run `verify-personal-layer.sh` → **GREEN** (`rtk gain` > 80%, plugins present); recreate again → confirm 0 re-install / 0 re-login (SC-006/SC-007). Separately confirm the absent-layer skip path (unset dotfiles → exit 0 notice, toolchain still works). **DONE 2026-07-14**: dotfiles repo `github.com/jumbleknot/mcm-dotfiles` (public) → RTK at persisted `~/.claude/tools/bin`, 15 plugins installed, `verify-personal-layer.sh` PASS; 2nd rebuild = 0 re-install; `rtk gain` **99.0%** (SC-006). (Minor: RTK auto-hook needs an interactive `rtk init -g` — manual `rtk` + compression proven.)
...
- [X] T034 Run the `quickstart.md` final validation **inside the container**: `pnpm nx e2e mcm-app` (web E2E — real dev path), `pnpm nx test mc-service && pnpm nx test:integration mc-service` (Rust, newly in-container), `rtk gain` > 80% — the constitution's per-feature E2E + RTK gate. **DONE 2026-07-14** via the CI-parity **containerized-BFF** path (`E2E_BFF_TARGET=dev-container` → :8082, ci-realm import; `pnpm nx e2e` Metro-mode superseded — even CI uses the container path): **104/104** core web specs (auth/collections/movies/session-timeout) green in a real Chromium; **148** mc-service unit tests pass; `rtk gain` **99.0%**. Agent/assistant specs excluded (need gateway+LLM, out of scope). Full recipe → HANDOFF + memory `project_mcm_038_devcontainer_full_toolchain`; needed the [[fix/038-devcontainer-compose-v5-parity]] Compose-v5 bake (baked in this PR).
