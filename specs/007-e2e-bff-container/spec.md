...
- **SC-005**: The security review reports zero unresolved High/Critical findings; production hardening is confirmed intact — **TLS 1.3, HSTS, CORS, security headers (CSP/X-Frame-Options/X-Content-Type-Options/Referrer-Policy), `Secure` cookies, and token validation** — not relaxed for tests; Medium/Low documented.
