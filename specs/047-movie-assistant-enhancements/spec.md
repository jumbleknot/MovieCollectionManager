...
- **FR-039**: A read of the member's own data that did not complete MUST NOT be presented as though it had. Specifically: a failed listing MUST NOT be presented as an empty one, a failed count MUST NOT be presented as zero, and a listing that stopped partway MUST NOT be presented as the whole. In every such case the assistant MUST say that it could not read the data and that the member can try again, and MUST NOT make any claim about what the member does or does not have. This applies wherever the assistant reads the member's collections or movies — navigating, organizing, answering questions, searching, importing and exporting alike.
  ...
  - *Relationship to FR-018, **corrected 2026-08-05 after live verification** — an earlier draft of this note claimed a failed dedup read creates the duplicates FR-018 forbids. It does not. mc-service's `(title, year)` uniqueness rejects the duplicate writes downstream, and the collection is left correct either way. What a failed read actually does is build the **preview** from an empty "what is already there": the member approves "2 movies will be added" and is then told "0 imported, 2 already up to date". The defect is an **approval given on a description of the change that is not true** — an integrity problem for the explicit-approval step (FR-037), not data corruption. Recorded this way because the weaker, accurate claim is the one that holds.*
