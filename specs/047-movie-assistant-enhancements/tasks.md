...
- [x] T098 Run the full web E2E regression: `E2E_BFF_TARGET=dev-container pnpm nx e2e mcm-app`
  ...
  - **Found two PRE-EXISTING failures — NOT PR B's, verified by bisect.** `agent-navigate-movie` (US6-AC1, `waitForURL` timeout) and `agent-add-external-link` (US5-AC1/AC2, element not found) fail **identically on `main` (56ed701)** with `main`'s own rebuilt images. Both specs, and the code they exercise, are untouched by PR B. They were almost certainly masked by the skip above rather than newly broken. **They need their own fix and should not block PR B, but must not be re-buried** — see [PRD-KnownFailingAgentE2E.md](../../docs/proposals/PRD-KnownFailingAgentE2E.md).
