...
- **FR-001**: Every externally-named Docker volume MUST follow `<context>-<role>-<engine>-data`; every network `<scope>-network`; every service/container `<context>-<role>` — where `<context>` is the owning component (`keycloak`, `mc-service`, `mcm-bff`, `movie-assistant` for resources of this specific assistant, `agent` for platform-level resources shared by any future agent such as the audit sink, `observability`).
...
- **FR-008**: Service/container DNS renames (Phase 2) MUST include a documented per-environment step to update the gitignored `.env` files, and every `.env*.example` in the repo MUST be updated.
