...
- [ ] **T001** [US1] Resolve `langfuse/langfuse:4` and `langfuse/langfuse-worker:4` to **manifest digests**
  on **Docker Hub** (FR-006 — not `docker.langfuse.com`) and record them in `research.md` with the date.
- [ ] **T002** [US1] Scan both digests with the gate's own criteria (`trivy image --exit-code 1 --severity
...
- [ ] **T003** [US1] Record per image: is **CVE-2026-75604 / GHSA-2xp9-vwfh-vxw4** present? Also record the
...
- [ ] **T004** [US1] **VERDICT** in `research.md`:
...
- [ ] **T005** [US2] Write a guard asserting that in **both** `compose.yaml` and `compose.prod.yaml`,
...
- [ ] **T006** [US2] **Verify GREEN** against the current tree.

---

## Phase 2: Local bring-up on Langfuse 4 + ClickHouse 25 (US-2)

- [ ] **T007** [US2] Move all three images in `compose.yaml` to tag + digest: `langfuse/langfuse:4`,
  `langfuse/langfuse-worker:4`, `clickhouse/clickhouse-server:25.12` (FR-004). Leave `postgres` and `redis`
  untouched.
- [ ] **T008** [US2] Recreate the `langfuse-postgres`, `langfuse-clickhouse` and MinIO volumes, bring the
...
- [ ] **T011** [P] [US2] Confirm `postgres` is still **16** and `unleash-postgres` is untouched — T005's
...
- [ ] **T012** [US3] Move the same three images in `compose.prod.yaml` (FR-004).
...
- [ ] **T016** [US4] **Delete** both `langfuse/*` entries from `security/infra-images/allowlist.yaml`
...
- [ ] **T017** [US4] Re-key or drop any allowlist entry whose key names `clickhouse…24.3` (FR-008) — an
  entry must not outlive the image it describes.
- [ ] **T018** [US4] Remove `allowedVersions: "<4"` from `renovate.json` packageRule 20 and **rewrite** its
  description to record the outcome; do not delete the text (FR-007).
- [ ] **T019** [US4] Extend `renovate-workflow.guard.test.mjs`: packageRule 20 must **either** still hold
