...
- [X] T019 [US1] Mobile Maestro: add a movie while leaving the language field EMPTY (the field is optional — US1's point). Extended `frontend/mcm-app/tests/e2e/mobile/movie-add.yaml` (the "014 US1" block — fills title+year ONLY, never touches the language input, and submits; the form must not block → proves optional-on-input. Then on the detail screen it asserts the **absent** value DISPLAYS as the neutral `—` placeholder (`movie.language || '—'`) → proves the neutral-placeholder AC; teardown). **LIVE-GREEN on the emulator** (full flow incl. SSO login → navigate → add-with-language → add-WITHOUT-language → empty submit succeeds → `—` display asserted → delete, all COMPLETED). **Required a FRESH emulator** (`-no-snapshot-load -gpu swiftshader_indirect`): a snapshot-loaded emulator's `adb reverse` listener flaps → device Chrome `ERR_CONNECTION_REFUSED` on Keycloak `localhost:8099`. Also had to temporarily relax `AGENT_RATE_LIMIT_REQUESTS` in `.env.local` (the CopilotKit dock's runtime-info poll 429'd → a LogBox overlay covered the form — the 013 pre-existing `runtime_info_fetch_failed`, not a US1 bug; reverted after).
...
- [X] T037 [US2] Implement the web import UI. **(Shipped UX deviates from the original "single import dialog" plan — Session-7 rework replaced it with type-to-start.)** `frontend/mcm-app/src/components/agent/request-import-file.tsx` (inline Choose/Cancel file picker via `utils/pick-file.ts`), `import-preview.tsx` (confirm-once summary: per-tab counts + whole-tab exclude toggles, pinned Approve), `render-import-report.tsx` (collapsible skipped/failed report), and `frontend/mcm-app/src/hooks/use-spreadsheet-import.ts` (upload → run → progress). useCallback-wrap dock handlers (react-compiler rule).
...
- [X] T047 [US3] Implement `frontend/mcm-app/src/app/bff-api/agent/export-download+api.ts` + `AGENT_ROUTES`/route-coverage-map. **(Shipped UX deviates from the original "export dialog" plan: export is assistant-driven — the node emits a `download_export` UI-action handled by `frontend/mcm-app/src/components/agent/ui-action-tools.tsx`; no separate `spreadsheet-export-dialog.tsx`/`use-spreadsheet-export.ts` was needed.)**
  - Verify GREEN: same command → passes.
