...
- [X] **T105 (#6, FR-030) — TMDB fail-closed**: web-api-mcp `_tmdb_key()` raises a clear config error when neither a per-request key nor a static fallback exists (was an unauthenticated TMDB call surfaced as "couldn't find it"); `_static_tmdb_key()` keeps startup priming tolerant. *Tests: `test_tmdb_key_raises_when_no_per_request_and_no_static_key`, `test_static_tmdb_key_primes_tolerantly_to_empty`.*
