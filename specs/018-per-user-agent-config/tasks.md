...
- [X] T006 Implement BFF Mongo connection singleton in `frontend/mcm-app/src/bff-server/mongo-client.ts` (lazy connect, scoped `MONGO_*` creds, returns the `bff_db` handle on the BFF's dedicated `mcm-bff-db` instance — see Phase 12); no secret in logs.
...
- [X] T021 [P] [US2] Write Python per-run injection unit test in `agents/movie-assistant/tests/unit/test_agent_config_injection.py`: `inject_agent_config` places provider/keys under `configurable`; node model build sources from `configurable` not `os.environ`; missing anthropic key → escalation degrades to base (R10). **Verify RED**: agent unit target (see docs/runbooks/agent-layer.md) → fails (injection absent).
...
- [X] T052 [P] Docs: add the new env vars (`AGENT_CONFIG_ENC_KEY`, `MONGO_*`, removed `MODEL_PROVIDER`/`OLLAMA_BASE_URL`/`ANTHROPIC_API_KEY`/`TMDB_API_KEY` from user-facing runtime) to [docs/runbooks/local-dev.md](../../docs/runbooks/local-dev.md) env table and the per-user-config flow to [docs/runbooks/agent-layer.md](../../docs/runbooks/agent-layer.md). Update repo-root `CLAUDE.md` if a new always-true rule emerges.
...
- [X] **SC-009 (SSRF)**: A user-supplied model base URL pointing at link-local/cloud-metadata — including the IPv4-mapped IPv6 form (`[::ffff:169.254.169.254]` → canonical `[::ffff:a9fe:a9fe]`) — is rejected at save/probe; loopback/LAN (incl. mapped loopback) is accepted; the allow-list, when set, restricts to listed hosts. *(T107 + the implementation-review SSRF canonicalization fix; `agent-config-ssrf.test.ts` incl. mapped-IPv6 regression. Residual: runtime Python `ChatOllama` fetch is unguarded + no DNS resolution — documented follow-up.)*
- [X] **SC-010 (at-rest binding)**: A stored blob encrypted for one (user, field) cannot be decrypted as another user/field — GCM auth fails. *(T108; `agent-config-crypto.test.ts` AAD context-binding.)*
- [X] **SC-011 (per-user model isolation)**: Every model stage (incl. intent classification) runs against the user's own provider/key; no shared model id leaks across a provider switch; an Ollama-only user never reaches a shared Anthropic key. *(T101/T102/T104; `test_runtime_env_*` + `test_supervisor_binds_per_run_agent_config_for_the_classifier`.)*
...
## Phase 12 — Dedicated BFF MongoDB instance (Decoupling remediation, 2026-06-19)

The implementation-review architecture pass found the BFF persisting its `user_agent_config` to **mc-service's `mc-db` instance** (reused for convenience). That is direct database access across a service boundary — a **constitution §Decoupling violation** ("Backend Services must be loosely coupled … avoiding direct database access across service boundaries") and it couples the BFF's deploy/scaling to a backend service's database. The BFF must own its store (as it already owns a separate Redis). Remediation:

- [X] **T113 — dedicated `mcm-bff-db` compose service**: add a standalone MongoDB (`mongodb/mongodb-community-server:8.0.8-ubi9`, **no replica set** — the BFF store does single-doc upserts only, so no transactions / `directConnection`) to `infrastructure-as-code/docker/bff/compose.yaml`, on `bff-network`, host port `127.0.0.1:27018:27017`, external volume `mcm-bff-db-data`, with a `mongosh` ping healthcheck. `mcm-bff`/`mcm-bff-dev` now `depends_on` it (healthy). Default profile (no profile) so `docker compose up -d` starts it alongside `mcm-redis`/`mc-db` for Metro/host dev + integration tests. First-time volume create added to root `compose.yaml`.
- [X] **T114 — repoint BFF Mongo config off `mc-db`**: `env.ts` defaults `MONGO_URL`→`mongodb://localhost:27018`, `MONGO_DB_NAME`→`bff_db` (own instance + own db, never `mc_db`); `.env.example` updated; `.env.local`→`mongodb://localhost:27018`, `.env.docker`→`mongodb://mcm-bff-db:27017` (standalone → the `directConnection=true` workaround is **removed**, no longer needed). The agent-config integration suites drive the live BFF over HTTP and assert via the in-process `store`, so both share `bff_db` on the dedicated instance (the env default) and self-clean by test-prefixed userId — the same shared-db pattern they used before (when it was `mc_db`), now on the BFF's OWN instance. (Mongo db-level test isolation à la the Redis db-1 pin is a noted follow-up — it needs the live BFF container to use the test db too.)
- [X] **T115 — verify**: `docker volume create mcm-bff-db-data` + `docker compose up -d mcm-bff-db` brings up the dedicated instance (healthy); the **full agent-config integration suite (7 suites / 38 tests)** is GREEN against it via Metro :8081 + dedicated Mongo (store/save/test/scoping/probes/run-revoked/route-auth); BFF unit **1104** + `tsc` green. Architecture doc + `local-dev.md` updated (dedicated instance node + edge + collections table + env table + the dropped `directConnection` note). *Note: routing these suites at the dev container (:8082) instead surfaces a host/container Ollama-reachability artifact (save-time probe 422 when the container can't reach the host's `localhost:11434`) — orthogonal to the Mongo change; run them against Metro on the host.*

> Supersedes the original Complexity-Tracking entry that reused the `mc_db` instance; `plan.md` Storage + Constitution Check (§Decoupling row) + Complexity Tracking updated accordingly. The `directConnection=true` gotcha (HANDOFF Load-bearing notes) no longer applies to the BFF — it was an artifact of reusing `mc-db`'s replica set.

---

## Phase 13 — In-session enable/disable reactivity (FR-031 / SC-012, manual-test bug, 2026-06-19)

Manual testing surfaced a UX bug: after enabling+saving a valid config the dock did **not** appear, and after disabling+saving the dock **stayed** (non-functional, since the BFF `/run` gate is authoritative-off) — both only corrected by signing out and back in. Root cause: `useAssistantConfig` was per-component local state, so the dock gate (`(app)/_layout.tsx` `AuthedAssistant`) and the Profile form (`movie-assistant-config.tsx`) each held an INDEPENDENT copy; the form's `save()→refresh()` updated only the form's copy, and the gate's copy only re-fetched on a `(app)`-layout remount (full reload / re-login). The web E2E missed it because the "dock appears" assertion follows a `page.goto` (full page load) that remounts the layout — masking the stale-gate path. Fixed TDD-first.

- [X] **T116 (TDD red) — shared-state regression test**: `src/hooks/unit-tests/use-assistant-config.test.tsx` — render a tree with the new `AssistantConfigProvider` wrapping TWO independent consumers (a gate reading `runnable`, a saver calling `save()`); mock `apiClient` GET→disabled then PUT+GET→runnable; assert the *gate* consumer flips to runnable after the *saver* saves (proves a single shared state, no remount). Fails until T117.
- [X] **T117 (impl) — promote config to shared context**: convert `use-assistant-config.tsx` to a context provider (`AssistantConfigProvider` + `createContext`/`useContext`, mirroring `use-auth`); `useAssistantConfig()` reads the context (throws if used outside the provider). Mount `AssistantConfigProvider` in `(app)/_layout.tsx` inside `AuthGuard`, wrapping both the `<Stack>` (which renders the Profile form) and `<AuthedAssistant/>` (the gate). No change to the hook's returned shape, so the form's call site is unchanged.
- [X] **T118 (E2E, de-mask) — same-session reactivity**: extend `assistant-config.spec.ts` so the enable case asserts the dock appears WITHOUT a full reload between save and assertion (in-app/SPA path, not `page.goto`), and the disable case asserts the dock disappears in-session. Run via the dev-container path (rebuild image first).
- [X] **T119 — verify**: `nx test mcm-app` (unit incl. T116) green; `tsc`/`lint` clean; dev-container web E2E (`assistant-config` + the standard assistant suite) green; mobile parity flow noted (issue #16, same CI gate as the other 018 flows).
- [X] **T120 (secret hygiene, manual-test bug) — block password-manager autofill on the API-key fields**: the Anthropic + TMDB key inputs (`secureTextEntry` → web `<input type="password">`) still drew the browser's built-in password-manager autofill/save UI: Chrome IGNORES `autocomplete="off"` on a password field. Fixed in `components/no-autofill-input.tsx` — a `secureTextEntry` field now emits `autocomplete="new-password"` (the only token Chrome honours to suppress its PM) instead of `off`, keeping the third-party `data-*ignore` attrs; a caller can still override `autoComplete`. Scope check: the only `NoAutoFillInput`+`secureTextEntry` fields in the app are these two API-key inputs (registration uses plain `TextInput` — the PM-allowed exception), so no login/fill-wanted field regresses. *Tests: `no-autofill-input.test.tsx` (+3: new-password on a secure field, ignores still present, caller override). RED→GREEN; full unit 1108, tsc/lint clean.* (Serves FR-006 / secret-hygiene FR-023.)
