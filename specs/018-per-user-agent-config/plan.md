...
**Storage**: New `user_agent_config` collection in a **dedicated, BFF-owned MongoDB instance** (`mcm-bff-db`) — **separate from mc-service's `mc-db`** so the BFF never reaches across a service boundary into a backend service's database (constitution §Decoupling: "avoid direct database access across service boundaries"). It mirrors how the BFF already owns a **separate** Redis cache. The BFF store does only single-document upserts (no multi-doc transactions), so `mcm-bff-db` is a **plain standalone `mongod`** (no replica set, no `directConnection`) — unlike `mc-db`, which needs a replica set for mc-service's cascade-delete transaction. Secrets stored as AES-256-GCM blobs (iv + auth tag + ciphertext). Redis remains the per-user cost/rate/thread store (unchanged).
...
| **Decoupling / No cross-boundary DB access** (Backend Services §Decoupling: "avoid direct database access across service boundaries") | ✅ PASS | The BFF persists to its **own dedicated `mcm-bff-db` instance**, NOT mc-service's `mc-db`. The BFF never opens a connection to a backend service's database — it mirrors the BFF's already-separate Redis cache. (An earlier draft reused the `mc-db` instance; that was a Decoupling violation, corrected to a dedicated instance.) |
...
**Structure Decision**: No new project. The feature extends three existing units — the **Expo BFF-Layer** (`frontend/mcm-app/src/bff-server` + `bff-api/agent/config`), the **Python gateway** (`agents/movie-assistant`), and the **web-api MCP server** (`mcp-servers/web-api-mcp`). Storage is BFF-owned (new `mongo-client.ts` + `agent-config-store.ts` → the **dedicated `mcm-bff-db` instance** added to `infrastructure-as-code/docker/bff/compose.yaml` alongside the BFF's existing Redis), explicitly **not** mc-service's `mc-db`, because per-user credential custody is a BFF-Layer responsibility, not movie-domain logic, and cross-boundary DB access is a constitution §Decoupling violation. The per-run credential path reuses the proven `X-*` header → ASGI-middleware → ContextVar → `config["configurable"]` bridge already used for the subject token and UI snapshot.
...
|---|---|---|
| BFF gains a **dedicated** MongoDB instance (`mcm-bff-db`) | Durable, per-user, encrypted credential storage scoped to the BFF's credential-custody role, in a store the BFF owns | **Redis** (current BFF store) has no first-class durability for long-lived config and conflates session-TTL data with permanent settings. **mc-service** owns the movie domain, not auth/BFF-adjacent credentials — routing user keys through it would leak credential custody into a domain service and add a network hop on the run hot path. **Reusing mc-service's `mc-db` instance** (an earlier draft) was rejected: it is direct DB access across a service boundary (constitution §Decoupling violation) and couples the BFF's deploy/scaling to a backend service's database. A dedicated standalone instance mirrors the BFF's already-separate Redis and stays a single-doc store (no replica set). |
