...
`docs/runbooks/agent-layer.md` + `agents/movie-assistant/README.md` + `specs/012-multi-agent-mvp/HANDOFF.md`. Key files: `agents/movie-assistant/src/nodes/organizer.py` (`plan_operations` @153, `_organize`), `runtime_nodes.py` `_build_organizer_node`. **Rebuild the gateway image after ANY agent-source change** (`node scripts/agent-stack.mjs --build`) — stale image = old code (this is exactly what bit this validation).
