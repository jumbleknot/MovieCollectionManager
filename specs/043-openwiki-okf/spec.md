...
- **FR-028**: The feature-completion checklist MUST require **checking for staleness** on every feature, and MUST require running the bundle update **only when staleness is reported**, including the resulting diff in the same change. The check MUST be free — no model call — so that features touching no cited document cost nothing. Rationale (measured 2026-07-27): an unconditional update is the wrong default because the tool advances its own update marker **only when wiki content changed**, so a run that correctly finds nothing to document leaves a stale marker and the *next* run pays full model cost again — indefinitely. The cheap path is reachable exactly when the wiki did change, and the expensive path runs when it did not.
- **FR-028a**: The staleness signal MUST be the conformance gate's own report-only warnings, so the trigger costs nothing beyond a check that already runs. The gate MUST report, without failing: (a) any concept whose cited source changed after the concept's own timestamp, and (b) **any canonical document that no concept cites**. Requirement (b) exists because (a) alone cannot detect a *newly added* document — there is no concept to compare it against — so drift-triggering on (a) by itself would let coverage (FR-007/SC-012) decay silently as new runbooks and decision records appear.
- **FR-028b**: The limits of the staleness signal MUST be documented where the checklist step is defined, so its blind spots are known rather than assumed away: it compares against **committed** history, so uncommitted work-in-progress is invisible; a concept carrying no timestamp never reports drift; and a concept citing an external URL never reports drift, because external links are deliberately never fetched (FR-013a).
...
- **SC-011**: The feature-completion checklist contains the staleness check and the conditional update step, and both paths are rehearsed during this feature: a run with staleness reported produces a reviewable diff, and a run with none reported completes **without a model call**.
- **SC-015**: The staleness check costs no model call and completes in under 10 seconds, so that gating every feature on it is free.
