...
- **FR-022**: The primary instruction file MUST NOT have existing content removed by this feature; it gains only the pointer and the correction in FR-023.
...
- **SC-007**: The primary instruction file's content is unchanged apart from the added pointer and the single environment correction — no existing line is deleted, and total growth stays under one kilobyte.
