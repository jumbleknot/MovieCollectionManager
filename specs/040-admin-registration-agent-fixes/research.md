...
- **Implementation finding (2026-07-16, blocks the "re-parse key" alternative)**: the spreadsheet-mcp upload store is **single-use** — `read_upload` DELETES `import:file:<handle>` immediately after the first successful read (`mcp-servers/spreadsheet-mcp/src/store.py`), so `parse_spreadsheet` consumes the upload handle on the FIRST import turn. Therefore checkpointing the **upload handle as a re-parse key does NOT work** (a later turn can no longer re-materialize the parsed data from it). The viable path is the other branch of the constraint: **add a NEW parsed-data transient store** (e.g. `import:parsed:<handle>` in the same Redis) that stashes the parsed `{tabs, collections}` under a fresh handle with a **session-long TTL**, exposed via two new spreadsheet-mcp tools (`stash_parsed(data) → handle`, `fetch_parsed(handle) → data` that **refreshes the TTL on each read** so an active multi-turn session never expires). `GraphState` then checkpoints only `import_handle` (the small opaque token) in place of `import_context`. This makes T024 a **cross-service** change (spreadsheet-mcp store + new tools + `runtime_nodes.py` import node + `graph.py` state + rebuild the spreadsheet-mcp/gateway images), and its end-to-end proof (T020) requires the live agent stack — not just the local unit suite. The T019 multi-turn unit test (a second clarification turn still resolves the handle) is the guard for the "must not expire mid-session" requirement.
