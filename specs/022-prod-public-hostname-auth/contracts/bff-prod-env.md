...
2. `KEYCLOAK_PUBLIC_URL=https://auth.example.invalid` (browser-facing issuer the BFF accepts).
...
7. Session cookies are `HttpOnly; SameSite=Strict; Secure`, host-only on `app.example.invalid` (no `Domain` attribute) — unchanged from code, asserted by existing unit tests.
