...
- [x] T008 [US1] Wire fail-fast secrets in `compose.prod.yaml`: `KC_BOOTSTRAP_ADMIN_PASSWORD=${KC_BOOTSTRAP_ADMIN_PASSWORD:?set in keycloak/.env.prod}`; **single-source DB password** (feature-022 follow-up): both the Postgres service (`POSTGRES_PASSWORD=${KC_DB_PASSWORD:?…}`) and keycloak-service (`KC_DB_PASSWORD=${KC_DB_PASSWORD:?…}`) interpolate the SAME `${KC_DB_PASSWORD}` — **no `secrets/*.txt` file-secret, no `POSTGRES_PASSWORD_FILE`** (the file-secret + `.env.local` dual-source were removed across dev+prod+CI). No inline literal, no `:-`/`??` default anywhere. (FR-020, FR-022, FR-025, data-model cross-entity invariants)
...
- [x] T024 [US3] **Single-source DB password** (feature-022 follow-up — supersedes the original file-secret invariant): `KC_DB_PASSWORD` is one var interpolated by BOTH the Postgres service and keycloak-service (dev sources it from `stacks/auth.env` via gen-dev-secrets; prod from gitignored `keycloak/.env.prod`). The `secrets/keycloak_db_password.txt` file-secret, `POSTGRES_PASSWORD_FILE`, and the `keycloak/.env.local` dual-source were removed everywhere (dev compose, prod compose, gen-ci-env.mjs, app-ci.yml). On a fresh DB volume any value works; an existing volume keeps its original password. (FR-022, data-model cross-entity invariants)
