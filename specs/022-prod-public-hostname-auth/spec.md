...
- **The production backend image and the mobile build pipeline may not exist yet.** The identity-provider slice (US1) can be authored and deployed independently now; the backend/app slices (US2) are authored now and deploy once the image/build pipeline produces artifacts.
- **Deploy orchestration and public-route publication are out of repo.** Defining deployment stacks, publishing the two public routes, and storing real secrets in the operator secret store are manual operator steps documented in the companion runbook, not code deliverables of this feature.
