...
- [~] T022 [P] Implement `web-api-mcp` tools (`search_title`, `get_movie_details`) → TMDB via `httpx`, Vault-injected key, outbound-only (no `backend-network`), typed `matchConfidence`, in `mcp-servers/web-api-mcp/src/tools.py`
  ...
  - **Verify GREEN**: same command → passes against **real TMDB** (cassette only for the LLM dimension elsewhere, never for TMDB itself — constitution §Test Type Integrity). **Implemented + RED verified + lint clean (ruff + mypy strict); test SKIPS without a key.** GREEN is **pending a real `TMDB_API_KEY`** in `mcp-servers/web-api-mcp/.env.local` (none exists locally yet). Tools: v3 `api_key` query param; `search_title`→typed `none|exact|ambiguous`; `get_movie_details`→`EnrichedMovieCandidate` (year from release_date, poster CDN URL, original-language english_name). Test asserts stable facts about TMDB id 603 (The Matrix, 1999).
...
- [~] T033 Register Nx targets for `movie-assistant`/`movie-mcp`/`web-api-mcp` (`test`, `test:integration`, `lint`, `build`, `deploy`) via `@nxlv/python`; verify the `agents` compose profile boots the full stack. **Partial:** the gateway is now containerized (`agent-gateway:latest` builds — Dockerfile fixed with `build-essential` for `annoy`) and runs on `backend-network`; the local container-BFF path uses `scripts/agent-gateway-local.ps1` (host Ollama, no `agent-db` since the graph uses MemorySaver until T024). **Still pending:** the `@nxlv/python` Nx target registration, and a full `--profile agents` boot (needs the `ollama-models` + `agent-db-data` external volumes + the ~19 GB model pull).
