...
- [ ] T037 [P] [US1] Web E2E in `frontend/mcm-app/tests/e2e/web/assistant-add.spec.ts` — add named movie → preview+approval; reject leaves unchanged; approve adds once; retry adds once (idempotency); add-to-nonexistent shows create+add in one preview. **DEPLOY-SIDE PROVEN: `test_gateway_add_e2e.py` (1 GREEN)** drives the FULL gateway AG-UI endpoint end-to-end (header→bridge→config→production nodes→real TMDB enrich→real movie-mcp→mc-service via real Keycloak exchange→interrupt→resume→write; gated pre-approval, persisted post-approval; LLM stubbed per constitution; exact title "Coherence" 2013). **Browser blockers remain:** (1) the dock **HITL approval UI doesn't exist** — build `approval_request` interrupt render + approve/reject → BFF `resume+api.ts` (CopilotKit `useInterrupt`/`useHumanInTheLoop`); (2) curator must map spoken collection → `target_collection_name`; (3) LLM/TMDB determinism in-browser (no cassette harness T032).
