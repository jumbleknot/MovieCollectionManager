...
- [X] T028 Implement BFF `src/app/bff-api/agent/run+api.ts` — AG-UI stream **passthrough proxy** (no event-shape translation); `requireAuth` → `requireMcUser`; sanitize UI state; mint subject token; rate/cost guard
