...
**Branch**: `012-multi-agent-mvp` | **Updated**: 2026-06-07 (session 6) | **HEAD**: `d8c4da5` | **Tree**: clean, all work committed.
(Latest: **US2 (organize by conversation) substantially DONE — backend + live integration + live web E2E.** T051 movie-mcp update/delete tools, T050a/b organize proposals + organizer plan→batch→**sequential chunked approval** (≤50, FR-009b) + approval-time re-validation (404→skipped_missing, FR-009a/SC-010), T053 wiring, T052 render_collection_summary. T047 organize integration **3/3 GREEN live** (real movie-mcp→mc-service), T048 organize web E2E **GREEN live (21.9s, qwen2.5 plan)**, SC-005 dev-container 95/4-skip. **MVP organize scope = multi-item REMOVE** (update/move are follow-ups; proposals/apply/movie-mcp `update_movie` already built). Remaining US2: **T049 mobile live run** (flow authored) + **T063 golden organize-plan exemplar**. Before: session 5 T069 ambiguous-add; session 4 T032 golden; session 3 US1 complete. Read the session-6 block + "Where we are".)

**START-HERE for the fresh session (read first):**
0. **US1 + US2 are functionally complete and proven live (web).** US1 add (incl. ambiguous-title disambiguation, T069) and US2 organize (multi-item remove) both run end-to-end through the real gateway. **Next picks:** finish US2's tail (**T049** mobile organize E2E — bring up the emulator per the runbook; **T063** golden organize-plan exemplar — `plan_operations` is a model decision, add a "plan" decision kind to the golden runner + record vs Claude), or **US3** (context-aware "this", T054–T059), or **Phase 6** gates (T031/T064 token-leak SC-004, T060 decline, T061 kill-switch, T062 expiry). The **organize MVP is remove-only**; update/cross-collection move is the natural next organize slice (the proposals/apply/movie-mcp `update_movie` layers already support it — only the organizer needs to build update `OrganizeOp`s + compose the full-replace payload from a read). See [[project-mcm-us2-organize]].
