...
```yaml
  ...
  unleash:
    ...
    environment:
      DATABASE_URL: postgres://unleash:***REMOVED***@unleash-postgres:5432/unleash
      DATABASE_SSL: "false"
      INIT_ADMIN_API_TOKENS: "*:*.***REMOVED***"
      INIT_CLIENT_API_TOKENS: "default:development.***REMOVED***"
    ports:
      - "127.0.0.1:4242:4242"
    networks:
      - backend-network
...
- [ ] **Step 2: Verify** — `docker compose --profile observability up -d unleash unleash-seed`; then
  `curl -s -H "Authorization: *:*.***REMOVED***" http://localhost:4242/api/admin/projects/default/features`
...
services:
  opensearch:
    ...
    environment:
      ...
      - "OPENSEARCH_INITIAL_ADMIN_PASSWORD=***REMOVED***"
    ulimits:
    ...
    networks:
      - backend-network
    healthcheck:
      test: ["CMD-SHELL", "curl -sk -u admin:***REMOVED*** https://localhost:9200/_cluster/health || exit 1"]
      interval: 15s
      timeout: 10s
      retries: 12
      start_period: 40s
...
- [ ] **Step 3: Verify** — `docker compose --profile audit up -d`; then
  `curl -sk -u admin:***REMOVED*** https://localhost:9200` → 200; and
  `curl -sk -u admin:***REMOVED*** https://localhost:9200/_nodes/jvm | grep heap_max_in_bytes`
  → ≈ `1073741824`.
