...
**Deep-load dock quirk (root-caused; minor follow-up, NOT a US3 blocker):** the CopilotKit dock's agent state **resets ~1–2 s after a turn when the app is freshly DEEP-LOADED on a non-home route** (`page.goto('/collections/:id')` / browser refresh on a collection) — the locally-added user message appears then vanishes, the run reaches the backend (200, cookies present) but the AG-UI response never renders, and a retry loop eventually expires the token (the BFF `no_token` + CopilotKit `runtime_info_fetch_failed` seen earlier were SECONDARY — the expired token after the stuck loop, not the cause). **Client-side in-app navigation (login → home → open a collection) does NOT trigger it** — the dock initializes on the home boot and stays mounted; the agent run works on the collection screen. Proven by a 3-way probe: HOME ✅, CNAV (in-app home→collection) ✅, deep-load COLLECTION ❌ (user msg `1`→`0`). This is US3-independent (reproduces with all US3 client changes disabled). **Real users are unaffected** (they reach collections via the FR-009 home→default-collection redirect = client-side nav), so the E2E navigates in-app via `openCollectionViaHome` (never deep-load a collection before driving the dock). Likely cause: a late app-tree remount on deep-route boot (Expo Router hydration / `(app)` AuthGuard / collection data-load settling) that re-inits the CopilotKitProvider. Follow-up: harden the dock against a deep-load remount (e.g. stabilize the provider / guard against the remount) so refresh-on-collection also works — logged, low priority.
