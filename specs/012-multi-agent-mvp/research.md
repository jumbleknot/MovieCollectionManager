...
**Live-validation status (2026-06-08, session 8)**: the full bridge is **proven working live** end-to-end at the backend — gateway diagnostics confirmed the sanitized collection snapshot flows client→BFF `/ui-state` (cached)→`/run` (`X-UI-Snapshot` header)→gateway middleware→`config["configurable"]["ui_snapshot"]`→organizer, and `_resolve_current_collection` matches the on-screen `collection_id` against `list_collections` and builds the add proposal (the interrupt fires). **US3-AC2 (clarify on home) passes web E2E live.** **US3-AC1 web E2E is BLOCKED by a PRE-EXISTING, US3-independent limitation: the CopilotKit assistant dock cannot run the agent from a non-home screen.** A minimal probe (open the dock on a collection screen, send a trivial "hello") fails identically with ALL US3 client changes disabled (UiStateProvider removed + the collection-route `useReportUiState` removed) — symptom: BFF `auth_failed: no_token` on `/bff-api/agent/run` + CopilotKit `runtime_info_fetch_failed` ("Failed to fetch" on `…/run/info`) + an empty dock (not even the local user message renders), while the SAME run from **home** succeeds (US1 `assistant-add` reject passes 19 s on the same stack). US3 is the first story that requires running the agent from a collection screen, so it surfaces the gap. The "this" resolution logic is correct and unit/integration/live-backend-proven; closing AC1's E2E needs a focused fix of the dock-on-non-home-screen agent run (likely the CopilotKit runtime `/info` probe + the short-lived `mcm_access_token` cookie / `createRefreshingFetch` interaction on a deep route) — best debugged interactively (browser network panel), see HANDOFF session-8.
