...
## R15 — Context-aware "this"/current-screen resolution (US3): how the sanitized `ui_snapshot` reaches the graph

**Problem**: US3-AC1 ("add \<movie\> to **this**" resolves the on-screen collection) needs the current screen's `collection_id` to reach the organizer's target resolution, while the constitution requires readable UI state to be **sanitized at the BFF (sole sanitization point)** before it reaches the prompt (R6/R12). The original `agent-bff-routes.md` contract sketched a clean `POST /run` body `{message, threadId, uiState}` the BFF would sanitize inline. **That body shape is obsolete**: T029 made `/run` host the **CopilotKit runtime** (`@copilotkit/react-native` → `runtimeUrl`), so the real request body is the opaque CopilotKit envelope (`{method, params, body: RunAgentInput}`) and `run+api.ts` just delegates to `copilotRuntimeNextJSAppRouterEndpoint(handleRequest)` — there is no clean `uiState` field to sanitize, and the body flows verbatim to the gateway through the runtime.

**Mechanism investigated** (CopilotKit 1.59.5 + `@ag-ui/client` 0.0.53, traced through installed source):

- Client readable state rides `RunAgentInput.context` (via `useAgentContext`) and surfaces at the gateway as `state["copilotkit"]["context"]` (prompt context) — NOT as a typed state field, and **unsanitized** (the BFF proxies the runtime body).
- `RunAgentInput.state` would land as graph input (`LangGraphAGUIAgent.prepare_stream` merges every top-level key of `input.state` into the graph input, then `filter_object_by_schema_keys` keeps only declared `GraphState` fields), but the BFF runtime overwrites the agent's state with the client's `input.state` (`handle-run.mjs: agent.setState(input.state)`), so a BFF-set `HttpAgent.initialState` is clobbered. ⇒ injecting via agent state is fragile.

**Decision**: carry the **BFF-sanitized** `ui_snapshot` out-of-band, **mirroring the proven subject-token bridge** (R3/T024 — already live end-to-end), NOT in the run body and NOT in checkpointed state:

1. **Client** maintains a structural UI-state context (`current_screen`, `collection_id`, `movie_id`, `active_filter_keys`, `nav_depth`) and **pushes the raw snapshot to `POST /bff-api/agent/ui-state`** on screen focus and immediately before sending a turn (the dock awaits the push so the snapshot is current at run time — no race).
2. **BFF `/ui-state`**: `requireAuth → requireMcUser → sanitizeUiState` (the existing sole-sanitization allowlist) → store the sanitized snapshot in the per-user cache (Redis, short TTL) → `204`. Added to the T028a auth-guard regression.
3. **BFF `/run`**: reads the stored sanitized snapshot for `user.id` and sets it as the `X-UI-Snapshot: <json>` **header** on the AG-UI `HttpAgent` (via `createMovieAssistantAgent({subjectToken, uiSnapshot})`) — alongside the existing `Authorization: Bearer` subject-token header. The header is independent of the (proxied, untouched) runtime body.
4. **Gateway**: a pure-ASGI `UiSnapshotMiddleware` (sibling of `SubjectTokenMiddleware`, NOT `BaseHTTPMiddleware`) captures `X-UI-Snapshot` into a request-local ContextVar; `IdentityAwareAGUIAgent.prepare_stream` injects it into `config["configurable"]["ui_snapshot"]` (task-safe, never checkpointed) — exactly the channel the real nodes already read identity from.
5. **Resolution** (organizer): the runtime organizer wrapper reads `ui_snapshot` from `config["configurable"]` and threads it into the node state; the pure organizer resolves "this" in **pure code** — `references_current_screen(text)` (keywords `this`/`current`/`here`, NOT an LLM call ⇒ no golden re-record, per T069g) + `_resolve_current_collection(ui_snapshot, collections)` (match `ui_snapshot.collection_id` against the user's collections when `current_screen ∈ {collection, movie-detail}`). An explicit current-screen reference takes precedence over the default-collection path; an unresolvable "this" (e.g. on `home`) → **clarify** (US3-AC2/FR-014), never guess.

**`GraphState` gains `ui_snapshot: dict | None`** so the organizer reads `state["ui_snapshot"]` and graph-level tests can pass it as input. It is non-secret structural data (data-model.md lists it as a GraphState field); the runtime wrapper overwrites it from `config` each run so a stale checkpointed value never leaks (and an absent snapshot → `None` → clarify, not a wrong guess). `state.forbid_token_fields` is unaffected (`ui_snapshot` is not a credential-named key).

**Scope** (this slice = US3-AC1/AC2): single-turn `"add <exact title> to this"` resolves the current collection (the E2E case — "Coherence"/"Inception"), and an unresolvable "this" clarifies. **Multi-turn ambiguous-"this"** (the "this" reference spoken on turn 1 of an *ambiguous*-title add, where the organizer is only reached on turn 2 after the pick) needs the same persistence treatment `target_collection_name` got (RC2) — logged as a follow-up, like US2 update/move. **`navigate_*` / `prefill_*` UI-action tools (T059)** are a separate generative-UI capability (uses the already-built `ui-action-authorizer`, T026) and are NOT required by US3-AC1/AC2 — scoped as a follow-up after the resolution ACs are green live.

**Alternatives considered**: (a) client `useAgentContext` straight to the gateway — rejected: bypasses BFF sanitization (constitution violation) and arrives as unstructured prompt context, not a clean field. (b) BFF rewrites the CopilotKit runtime body to sanitize an embedded snapshot — rejected: fragile coupling to the opaque envelope shape; the header bridge needs no body parsing. (c) `HttpAgent.initialState` (Channel A) — rejected: clobbered by `agent.setState(input.state)` in the runtime. (d) LLM-classified "this" detection — rejected: violates code-orchestration + forces a golden re-record; keyword detection is deterministic and sufficient.

**Live-validation status (2026-06-08, session 8) — US3 web E2E COMPLETE (AC1 + AC2 GREEN live) + SC-005 additivity 95/95.** The full bridge is proven end-to-end: client→BFF `/ui-state` (cached)→`/run` (`X-UI-Snapshot` header)→gateway middleware→`config["configurable"]["ui_snapshot"]`→organizer→`_resolve_current_collection` matches the on-screen id→proposal→interrupt→approve→movie-mcp→mc-service. `assistant-context.spec.ts`: **AC1** ("add Coherence to this" on a collection screen → resolves the on-screen collection, adds once, never creates a "this" collection) + **AC2** (clarify on home) both GREEN (~25 s). SC-005 dev-container regression **95 passed / 6 skipped** (the 6 = prod-gateway agent specs) — US3 changes are fully additive.

**Deep-load dock quirk (FULLY root-caused 2026-06-08; documented WON'T-FIX-NOW — effectively zero real-user impact):** the CopilotKit dock's agent resets its `messages` **~0.7–1 s after the first turn when the app is freshly DEEP-LOADED on a non-home route** (`page.goto('/collections/:id')` / browser refresh / direct URL / bookmark on a collection or movie) — the locally-added user message appears (T+300 ms) then vanishes (T+1 s), the run reaches the backend (200, cookies present) but the AG-UI reply never renders. **It is NOT a remount** (an earlier guess): instrumentation proved the root `RootLayout` / `AuthProvider` / `AssistantProvider` all stay mounted across the turn (the 6 `RootLayout MOUNT`s seen earlier were global-setup's OAuth login navigations, not a loop). It is a **one-time CopilotKit runtime/agents-load that resets the agent's messages shortly after the panel first renders+uses the agent**, racing a sub-second send. **Decisive evidence:** dwelling 6 s after opening the dock before sending → works (the load settles); sending immediately → message lost. **Client-side in-app navigation (login → home → open a collection) is unaffected** (3-way probe: HOME ✅ · in-app-nav ✅ · deep-load ❌); US3-independent (reproduces with all US3 client changes disabled). **Real users are effectively never affected** — a human cannot open the dock + read + type + send within the ~1 s window, and the common path (FR-009 home→default redirect) is in-app nav. The E2E navigates in-app via `openCollectionViaHome` (never deep-load a collection before driving the dock). **Fix attempts that did NOT close it** (2026-06-08): warming `useAgent`/`useCopilotKit` in the always-mounted dock root; always-mounting `AssistantPanel` (rendering its UI only when open — the agent isn't *used* until the UI renders, so init didn't settle early). The only proven mitigation is real elapsed time after the panel's first open-render. **Candidate fix for a future pass** (not done — risk/effort unjustified for a ~zero-impact edge): render the panel UI hidden/offscreen from boot so the agent is *used* + the runtime-load reset settles before the user can open+send, OR gate Send on a CopilotKit readiness signal (no clean public one found). Logged low priority.
