...
**Still to do (T029 remaining):** convert `bff-api/agent/run+api.ts` from raw passthrough to a CopilotKit-runtime endpoint (add `@copilotkit/runtime`, verify it runs under `@expo/server`), then the live web E2E (Playwright) against the running gateway + BFF + Ollama. The client overlay (provider + dock) is built, type-correct, mounted (auth-gated), and render-tested; the server transport is proven.
