...
**Alternatives rejected**: filter in application code after fetching all (wasteful, breaks keyset pagination).

**Implementation deviation (2026-06-04, documented per SDD).** The shipped implementation does NOT remove the `owner_id` parameter from the `MovieRepository` read/delete methods. Instead each handler authorizes via the ACL helper and then passes **`collection.owner_id`** (the collection owner, resolved *after* authorization) to the repository — never the caller's identity. This satisfies FR-004 (the query no longer keys off the caller's identity; it keys off the collection owner, and after FR-005 every movie in a collection shares that owner, so the existing `{ collectionId, ownerId }` predicate returns exactly the collection's movies — and a seeded viewer/contributor reads correctly because the handler passes the collection owner, not the viewer). Rationale for the deviation: it delivers identical behavior for all in-scope requirements (cross-tenant denied via the helper; owner reads unchanged; seeded-role reads work) while avoiding a signature change that would ripple through six `mockall` mock blocks and the adapter queries — materially lower churn and risk. The only theoretical difference (a row whose stored `ownerId` drifted from the collection owner would be excluded) is moot under the clarified fix-on-write-only scope (no drift exists today). Verified by the `dac_authorization` integration suite (8/8, incl. the seeded-viewer read).
