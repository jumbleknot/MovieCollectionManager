...
- [ ] T018 Web E2E regression judgment call — per the constitution/Final Validation Checklist, web E2E is normally required for every feature. This feature changes **no** dev/app runtime (no BFF or mc-service image, no `compose.yaml`; only prod `compose.prod.yaml` port binds, the prod Mongo entrypoint, and docs), so the dev-path web E2E would exercise entirely unchanged behavior with no container to rebuild. Confirm with the maintainer whether to run `pnpm nx e2e mcm-app` anyway; default to running it if there is any doubt. Record the decision in the PR.
- [ ] T019 Open a PR from `028-prod-reboot-resilience` to `main` on the **`origin` (Forgejo) remote** (guardrails* + app-ci* checks) via the GCM-credential API call in CLAUDE.md. PR body: summary of US1–US4, the T003 restart-policy result, the T012 backend-network finding, the T018 E2E decision, and a note that the operator performs the single validation reboot (quickstart §D) after merge + Komodo sync.
