...
| **Model-provider scoping — BOTH providers stay supported** | PASS, and gated on both sides. The self-hosted provider remains the default (`MODEL_PROVIDER` unset → Ollama) and the hosted one remains the documented fallback; neither is removed, deprecated or made conditional. One message shape serves both (R1), and each has its own blocking gate: `guardrails` replays the Ollama tier on every PR, `test:golden-live` exercises the Anthropic tier before every deploy. FR-021–024, SC-008. |
...
| **The assistant stops working on one of the two providers** | Cannot happen silently. One message shape serves both, verified at the conversion code of each (R1), and **each provider has its own gate**: `guardrails` replays the Ollama tier on every PR, `test:golden-live` runs the Anthropic tier before every deploy. FR-021/022 make this a requirement rather than a side effect. |
