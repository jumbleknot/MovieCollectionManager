...
- **FR-032**: After two consecutive weeks of incident-free daily use, the configuration MUST be collapsed to a single description and the nested-engine path MUST no longer be offered for assistant sessions, while remaining available for non-assistant use and as the mobile-emulator fallback.
