...
- [X] T023 [US4] Generalize the fallback detector (FR-016) from just `E2E_TEST_PASSWORD` to a curated name set — `E2E_TEST_PASSWORD` + `ANTHROPIC_API_KEY` + `TMDB_API_KEY` — matching the JS `?? / ||` idiom and the `get/getenv/cfg/_cfg("NAME", "x")` getter-default idiom (RED via narrowing the set → the ANTHROPIC/TMDB positives fail; GREEN restored). The env-**setting** idiom `monkeypatch.setenv("NAME", "x")` and the out-of-set `CI_*_SECRET` throwaways are self-tested as clean negatives. Full scan stays green.
  - **Deferred (recorded in spec §Deferred)**: the `export-ci-realm.mjs` `CI_*_SECRET` throwaways stay as documented 023 fixtures (out of the name set, not fail-clean); the committed `ci-realm.json` plaintext throwaway credentials are a **separate follow-up feature** (touches the 023 CI realm-import path).

**Checkpoint**: The live E2E password is gone from the tree; every consumer is fail-clean; the gate blocks regression for the full E2E credential set.
