...
- Q: What is the schedule cadence and trigger? → A: **On merge to the main branch, debounced, plus manual dispatch.** Maintenance follows the work that caused the drift, so each run is small and reacts to a known change set. No weekly sweep. Accepted costs: contention with the capacity-constrained runner, and generation volume that scales with merge rate — both bounded by the per-run page and wall-clock budgets (see the amendment below) and by the run being off the merge-gating path.
...
- Q: What bounds the amount of generation a single maintenance run performs? → A: ~~An explicit per-run spend ceiling.~~ **SUPERSEDED by the amendment below** — the premise was wrong.
- Q: **[Amendment]** The spend ceiling turned out to be unmeasurable. What bounds a run instead? → A: **A per-run page budget and wall-clock budget, whichever is reached first.** Phase 0 research established that the generator emits no token or cost data of any kind: its full option list carries no usage or budget flag, a search of its distribution for token-accounting fields returns nothing, and its telemetry payload has no token counts. Feature 043 never measured cost either — every figure it recorded is wall-clock. A spend ceiling would therefore have been enforced against a fabricated estimate while *reading* as a monetary guarantee. Pages and elapsed time are both directly observable, so the budget is now stated in the units the repository can actually measure. Three consequences: the page count MUST come from what actually landed in the tree rather than from anything the generator reports about itself; the wall-clock budget now bounds runner occupancy directly, which contributes to the non-contention requirement instead of being orthogonal to it; and **no requirement in this feature asserts a monetary bound**, because nothing here can measure one.
...
- **A run produces far more than expected.** The budgets must stop it before the next slice starts, not report the overrun afterwards.
- **The generator over-produces within a slice**, ignoring the page list it was given. Nothing prevents this — the page count is an instruction in free text, not a constraint the tool enforces — so the budget must count what landed in the tree, never what the generator claims.
- **A single slice alone exhausts a whole budget.** The run cannot subdivide further and cannot interrupt work in flight, so the one-slice overshoot must be declared up front rather than discovered in production.
...
- **FR-011**: An automated maintenance run MUST be bounded by two declared budgets — a **maximum number of pages** and a **maximum wall-clock duration** — whichever is reached first. Both MUST be enforced at slice granularity: the counters are checked before each slice is started, and a slice that cannot start within budget MUST NOT be started; it carries forward to the next run instead.
- **FR-011a**: Because a slice already in flight is not interrupted, the effective bound is each budget plus at most one slice. That overshoot MUST be bounded and its worst case stated, not left open-ended.
- **FR-011b**: Both counters MUST derive from **verified observation**: pages counted from what actually landed in the working tree, and elapsed time measured by the run itself. The generator's own account of what it produced MUST NOT be used — the same reasoning as FR-005, applied to the budget.
- **FR-011c**: The wall-clock budget bounds how long a run occupies the build runner, and therefore contributes directly to FR-019's non-contention requirement rather than being orthogonal to it. FR-019 additionally requires that concurrent runs cannot accumulate.
- **FR-011d**: Neither budget is a proxy for monetary cost, and no requirement in this feature asserts one. The generator reports no token or cost data, and the repository holds no cost measurements to calibrate against — a stated monetary bound would be a fabrication presented as a guarantee.
...
- **SC-006**: No maintenance run exceeds its declared page budget or wall-clock budget by more than one slice, measured across every run in the validation period, and work stopped at a budget appears in the next run's plan.
- **SC-006a**: A run that stops at a budget reports that outcome distinguishably from a failure, and the stop is driven by pages observed in the working tree rather than by anything the generator reported about itself.
...
- **Costs of merge-triggered maintenance are accepted.** Runs contend with the capacity-constrained runner and generation volume scales with merge rate. Both are bounded by the per-run page and wall-clock budgets (FR-011) and by quiet-branch debouncing collapsing bursts into a single run; the wall-clock budget bounds runner occupancy directly (FR-011c), with the run also sitting off the merge-gating path. **Monetary cost is not bounded by any requirement here** (FR-011d) — the generator reports none and the repository measures none.
