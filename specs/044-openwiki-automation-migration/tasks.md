...
- [x] T001 [P] Create planner/verifier fixture tree in `scripts/__tests__/fixtures/wiki-maintain/` with: a conformant mini-bundle, a partially-written bundle, and a bundle with one new and one existing area
  - **Type**: New file · **Risk**: None
  - **Done when**: fixtures load offline with no network and no `ANTHROPIC_API_KEY`

- [x] T002 [P] Create governance fixture tree in `scripts/__tests__/fixtures/openwiki-governance/` with: valid `policy.yaml`/`protected.yaml`, an unclassified path, a reworded protected passage, a deleted protected passage, a protected passage on a `resource`-bearing concept, a concept that is neither derived nor authoritative, and a `CLAUDE.md` carrying stray prose
  - **Type**: New file · **Risk**: None
  - **Done when**: each fixture isolates exactly one G-rule violation

- [x] T003 Record the pre-trim baseline in `specs/044-openwiki-automation-migration/EVIDENCE.md`: `CLAUDE.md` line count, byte size, section count, and the bundle's concept count
...
- [x] T004 Author `openwiki/policy.yaml` declaring the per-path regeneration policy exactly as specified
...
- [x] T005 [P] Extend `openwiki/INSTRUCTIONS.md` with the new exclusions and the protection rule
...
- [x] T006 [P] Document the canonical-home routing rule in `openwiki/INSTRUCTIONS.md`
...
- [x] T007 Write the run-record module test in `scripts/__tests__/wiki-maintain.test.mjs`
...
- [x] T008 Implement the run-record module in `scripts/wiki-maintain.mjs`
...
- [x] T009 [US1] Write the slice-bounding test in `scripts/__tests__/wiki-maintain.test.mjs`
...
- [x] T010 [US1] Implement the planner in `scripts/wiki-maintain.mjs`
...
- [x] T011 [P] [US1] Write the offline-planning test in `scripts/__tests__/wiki-maintain.test.mjs`
...
- [x] T012 [US1] Implement `--plan` and `--json` per [contracts](contracts/cli-contracts.md) C1 in `scripts/wiki-maintain.mjs`
  - **Prerequisite**: T011 verified RED
  - **Verify GREEN**: same command → 0 failures

- [x] T013 [P] [US1] Write the run-message rendering test in `scripts/__tests__/wiki-maintain.test.mjs`
...
- [x] T014 [US1] Implement run-message rendering in `scripts/wiki-maintain.mjs`
...
- [x] T015 [US1] Write the zero-page detection test in `scripts/__tests__/wiki-maintain.test.mjs`
...
- [x] T016 [US1] Write the conformance-regression test in `scripts/__tests__/wiki-maintain.test.mjs`
...
- [x] T017 [US1] Implement the verifier in `scripts/wiki-maintain.mjs`
...
- [x] T018 [US1] Write the policy-write enforcement test in `scripts/__tests__/wiki-maintain.test.mjs`
...
- [x] T019 [US1] Implement the policy-write guard in `scripts/wiki-maintain.mjs`
...
- [x] T020 [US1] Write the budget-guard test in `scripts/__tests__/wiki-maintain.test.mjs`
...
- [x] T021 [US1] Implement the budget guard in `scripts/wiki-maintain.mjs`
...
- [x] T022 [US1] Write the resume test in `scripts/__tests__/wiki-maintain.test.mjs`
...
- [x] T023 [US1] Implement `--execute`, backlog persistence, and exit codes 0/1/2/3 per C1 in `scripts/wiki-maintain.mjs`
  - **Prerequisite**: T022 verified RED
  - **Verify GREEN**: `node --test scripts/__tests__/wiki-maintain.test.mjs` → 0 failures

- [x] T024 [US1] Implement `--selftest` in `scripts/wiki-maintain.mjs`
...
- [x] T025 [US1] Add the `wiki-plan` and `wiki-maintain` Nx targets to `infrastructure-as-code/project.json`
...
- [x] T026 [US2] Write the marker-advance test in `scripts/__tests__/wiki-maintain.test.mjs`
...
- [x] T027 [US2] Implement marker advance and outcome classification in `scripts/wiki-maintain.mjs`
...
- [x] T028 [P] [US2] Write the proposal-lifecycle test in `scripts/__tests__/wiki-maintain.test.mjs`
...
- [x] T029 [US2] Implement the proposal client in `scripts/wiki-maintain.mjs`
...
- [x] T030 [P] [US2] Write the missing-event-document test in `scripts/__tests__/wiki-maintain.test.mjs`
...
- [x] T031 [US2] Implement missing-event-document detection in `scripts/wiki-maintain.mjs`
...
- [x] T032 [P] [US2] Write the workflow-shape guard test in `scripts/__tests__/wiki-maintain.guard.test.mjs`
...
- [x] T033 [US2] Create `.forgejo/workflows/wiki-maintain.yml`
...
- [x] T034 [US2] Wire the feature-042 failure digest into `.forgejo/workflows/wiki-maintain.yml`
...
- [x] T035 [US2] Confirm `.forgejo/workflows/wiki-maintain.yml` is **not** added to branch protection required contexts
...
- [x] T036 [P] [US4] Write the local-parity test in `scripts/__tests__/wiki-maintain.test.mjs`
...
- [x] T037 [US4] Implement `--since`, `--max-slices`, and `--dry-run` in `scripts/wiki-maintain.mjs`
  - **Prerequisite**: T036 verified RED
  - **Verify GREEN**: same command → 0 failures

- [x] T038 [P] [US4] Write the operator runbook `docs/runbooks/wiki-maintenance.md`
...
- [x] T039 [P] [US3] Extend `scripts/__tests__/relocated-docs-links.test.mjs` for the agent-layer move
...
- [x] T040 [US3] Move `docs/agent-layer.md` → `docs/runbooks/agent-layer.md` and update every inbound reference in lockstep
...
- [x] T041 [P] [US3] Write policy-validation tests (G1–G4) in `scripts/__tests__/check-openwiki-governance.test.mjs`
...
- [x] T042 [P] [US3] Write protection tests (G5–G7) in `scripts/__tests__/check-openwiki-governance.test.mjs`
...
- [x] T043 [P] [US3] Write index tests (G8–G10) in `scripts/__tests__/check-openwiki-governance.test.mjs`
...
- [x] T044 [P] [US3] Write the concept-classification test (G11) in `scripts/__tests__/check-openwiki-governance.test.mjs`
...
- [x] T045 [US3] Write the fingerprint-normalization test in `scripts/__tests__/check-openwiki-governance.test.mjs`
...
- [x] T046 [US3] Implement `scripts/check-openwiki-governance.mjs`
...
- [x] T047 [US3] Add governance gate steps to the existing `okf` job in `.forgejo/workflows/guardrails.yml`
...
- [x] T048 [P] [US3] Add the `okf-governance` Nx target to `infrastructure-as-code/project.json`
  - **Type**: Config · **Spec reference**: contracts C5
  - **Done when**: `pnpm nx okf-governance infrastructure-as-code` runs keyless and offline

### The trim

- [x] T049 [US3] Plan the relocation from `CLAUDE.md` and review it before spending anything, via `scripts/wiki-maintain.mjs --plan`
...
- [x] T050 [US3] 💰 Execute the relocation of `CLAUDE.md` content into `openwiki/` through the slice machinery (`scripts/wiki-maintain.mjs --execute`)
...
- [x] T051 [US3] Populate `openwiki/protected.yaml` with the `authoritative:` list and a fingerprint per relocated load-bearing passage
...
- [x] T052 [US3] Rewrite `CLAUDE.md` as a thin index
...
- [x] T053 [P] [US3] Reconcile the other assistant-facing surfaces — `AGENTS.md`, `opencode.json`, `.claude/`
  - **Spec reference**: FR-033, SC-011
  - **Verify GREEN**: `pnpm nx okf-governance infrastructure-as-code` → exit 0 (G10)

- [x] T054 [US3] Validate the destination rule against six real subjects in `specs/044-openwiki-automation-migration/EVIDENCE.md`
...
- [x] T055 [US3] Complete `specs/044-openwiki-automation-migration/EVIDENCE.md`
...
- [x] T056 [P] Verify no credential leaked into generated content under `openwiki/`
...
- [x] T057 [P] Confirm every always-on gate in `.forgejo/workflows/guardrails.yml` remains keyless and fail-closed
  - **Spec reference**: FR-026, SC-014
  - **Done when**: the `okf` job's steps run with no `${{ secrets }}` reference

- [x] T058 [P] Run the full script unit suite as CI does, over `scripts/__tests__/*.test.mjs`
  - **Verify GREEN**: `node --test scripts/__tests__/*.test.mjs` → 0 failures

- [x] T059 Confirm V12 drift remains report-only in `scripts/check-openwiki-okf.mjs`
  - **Spec reference**: FR-036
  - **Done when**: the file is unchanged and drift still never affects the exit code

- [x] T060 [P] Verify no egress or telemetry widening in `.devcontainer/init-firewall.sh` and `.forgejo/workflows/wiki-maintain.yml`
...
- [x] T061 [P] Verify credential hygiene across `.forgejo/workflows/wiki-maintain.yml` and the published digest
...
- [x] T062 [P] Verify the maintenance proposal PR is gated like a human-authored change by `.forgejo/workflows/guardrails.yml`
...
- [x] T063 Verify the relocation of `CLAUDE.md` is revertible in one step
...
- [x] T064 Final validation checklist across `frontend/mcm-app` and `backend/mc-service`
...
- [x] T065 Update `docs/runbooks/agent-layer.md` and `CLAUDE.md`'s index for anything learned during implementation
