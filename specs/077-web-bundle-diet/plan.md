...
|---|---|---|
| **Frontend UI & UX — Performance Budgeting** ("JavaScript bundles must be lazy-loaded") | **Directly served** | This feature is the first lazy-loading of a JS bundle in the app. The same principle's 2-second-3G target remains unmet afterwards (see spec Assumptions) — this narrows the gap by 57%, it does not close it. Recorded in Complexity Tracking. |
...
|---|---|---|
| The constitution's "no page shall exceed a 2-second time-to-interactive on simulated 3G" remains unmet after this feature (≈37 s of transfer for the entry chunk alone). | The residue is `expo-router` (427 KB), `react-native-web` (276 KB), `react-dom` (175 KB) and `@tamagui/web` (120 KB) — the cost of rendering any route at all in this stack. Meeting 2 s would mean changing the rendering stack, which is not what item #558 asks for and is not affordable here. | Route-level splitting was measured at a ~68 KB ceiling (6% of the bundle is app code) and cannot close the gap. Deferring framework code is not possible — it is needed for first paint. The honest position is a 57% reduction plus a budget that stops further erosion, with the residual gap recorded here rather than hidden. |
