...
function emitProposedAllowlist(findings) {
  ...
  const entries = blocking.map((f) =>
    `- image: "${esc(f.image)}"\n  id: "${esc(f.id)}"\n  justification: "Baseline (035): pre-existing ${f.severity} in ${f.pkg} — awaiting Renovate base-image bump (fix ${f.fixedVersion})."\n  addedBy: "seed"\n`
  );
  ...
  mkdirSync(REPORT_DIR, { recursive: true });
  writeFileSync(resolve(REPORT_DIR, 'allowlist.proposed.yaml'), header + entries.join(''));
}
