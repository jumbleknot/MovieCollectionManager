...
Alpine's `openssl-dev` ships only dynamic libraries, so a musl target is unable to
link `-lssl` statically. The fix is a musl-conditional dependency — never an unconditional one, which
